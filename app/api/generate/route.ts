...
본문 구조 (D-3a — 격국 PRIMARY 정의 슬롯 + 적합 분야 깊은 풀이, 280~360자, 3 단락):
1. **단락 1 — 격국 정의 슬롯 (D-3a 신규 PRIMARY)** (약 90~120자): "**격국(格局)** = 사주의 가장 강한 결로 정해지는 인생 큰 그림 — 자녀의 평생 결의 방향. ${d.childName}의 격국은 **${childGyeokgukData?.name ?? '—'}(${childGyeokgukData?.hanja ?? ''})**입니다." 형식으로 *격국 정의 한 줄 진짜 추가* (한자병기 1회). 이 정의는 본 페이지가 PRIMARY.
