...
★★★ G14 v2 (2026-05-14) — 풀 대명사 단독 도입 절대 금지
- "그 기운"·"그 결"·"앞서 본 결"·"타고난 결"·"본인 결"·"중심 기운"·"관계의 자리"·"내면의 자리"·"마음 안쪽의 자리" 같은 풀 대명사는 **이미 같은 문맥에서 한자 토큰으로 명시한 인자가 재등장할 때(3회째+)만** 사용.
- **새 인자를 본문에 도입할 때 풀 대명사 단독 사용 절대 금지.**
- 예 금지: "${cnh}의 관계의 자리는 그 기운이에요. 이 결은 땅에 단단히 뿌리내린..." — "그 기운"이 무엇을 가리키는지 본문 맥락에 없음.
- 예 정답: "${cnh}의 관계의 자리는 ${d.childName}의 일지 오행 그대로의 결이에요. 이 결은 ..." 또는 직전에 한자 토큰을 명시.
- "...는 그 기운이에요" / "...는 그 결이에요" 패턴은 무조건 직전에 한자 인자명 명시 또는 한자 자체로 대체.
- 자동화 샘플에서 반복된 표현 금지: "그 결 기운", "그 흐름 기운", "앞서 본 결", "앞서 본 신살", "본인 결이 옅게 자리", "타고난 결이 옅게 자리"를 그대로 쓰지 말 것. 반드시 "식상", "인성", "비겁", "관성", "일주", "용신", "문창귀인"처럼 실제 인자명으로 바꿔 쓴다.
- "옅게 자리"는 보고서 전체에서 최대 2회만 허용. 같은 약함을 반복해야 하면 "표현 통로가 좁다", "자기 기준을 세우는 데 시간이 든다", "외부 반응을 먼저 살핀다", "혼자 밀어붙이는 힘이 늦게 켜진다"처럼 행동 문장으로 번역한다.
...
[시그너처: 무기 3가지 나열형] — 무기 1개 명명·영화 장면 묘사 절대 X. ${cnh}의 무기 3개를 각각 한 단락씩 산문으로 나열 + 각 무기마다 [[ ]] 강조 평판/멘트 1회 = 최대 3회. 빠름/깊음, 많음/진심 같은 일반 결론 반복 금지.
구성: ① 단정 — "${cnh}에게 오래 남을 무기는 [구체 무기명 1 — 자녀 일주에서 도출]이에요." → ② 첫 번째 무기 단락 (180~220자) — "일주는 ${cnh}의 가장 근본적인 본질이에요. ${cnh}의 일주는 [단단하고 진중한 / 부드럽고 받아주는 / 결단력 있고 추진하는 / 섬세하고 헤아리는] 결을 담고 있어요" 톤. 일주(60갑자 또는 일간·일지)·일간 오행·신살(괴강 = 단호 / 양인 = 결단 등) + 신강신약 결합. 또래가 휩쓸릴 때 ${cnh}이 어떻게 자기 자리를 지키는지 + 어릴 때는 [[ ]] 평판으로 오해받기도 하는 결(예: [[고집이 세다]] / [[융통성이 없다]] / [[너무 무르다]] 자녀 결에 맞는 한 줄) → ③ 두 번째 무기 단락 (140~180자) — 신뢰감·약속 지키는 결(관성 강) / 마음으로 받아주는 결(인성 강) / 빠른 판단 결(식상 강) 등 자녀 사주 인자에서 도출. 친구·가족·미래 직장에서의 평판 [[ ]] 강조 1회 (예: [[걔는 믿을 수 있어]] / [[걔한테 부탁하면 끝까지 해]] 자녀 결에 맞는 한 줄) → ④ 세 번째 무기 단락 (140~180자) — 깊이 보는 눈(인성 강) / 분위기 살리는 손(식상 강) / 사람 챙기는 마음(재성 강) 등. 그 결이 분석·본질 보기·관계 다루기에서 어떻게 강점이 되는지. → ⑤ 끝문장은 세 무기가 실제로 쓰이는 장면으로 묶는다: "이 세 무기는 훗날 ${cnh}이 [사람/문제/작업] 앞에서 [어떻게 버티고/읽고/끝내는지]로 드러납니다." 무기 3개는 자녀 사주 인자에서만 도출 — 일반론 X.
