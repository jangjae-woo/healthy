...
- **모든 본문 마지막 한 문장은 자녀 영향(child impact)으로 마무리**: "${d.childName}${d.childGender === '남' ? '군' : '양'}이(은/는) ~한 자녀로 자라간다" 또는 "~한 결로 빛난다" 형태.
...
- ★★★ **모든 페이지 본문에 핵심 단어/구절을 \`**굵게**\`로 강조** — 가독성을 위한 규칙. 한 페이지(280~360자)당 **정확히 2~3개**만 \`**...**\` 마크다운으로 감싸기. 그 이상은 강조 효과가 죽고 페이지가 한 색으로 도배되어 보이므로 절대 금지.
...
- 🔴 강 부정 한자 절대 X: "상극(相剋)", "충(冲)", "양인살", "공망", "괴강", "형(刑)", "칠살".
...
🔴 **두 가지 약점을 한 문장으로 묶지 말 것** — 비겁 약 + 관성 약 같은 두 약점을 묶으면 부정문 구조가 꼬여 강 양상으로 역해석되는 회귀가 발생함. 각 결은 **독립 문장**으로 분리해서 서술.
🔴 약 결을 강 양상 어휘로 묘사하면 명리 역해석 — 절대 금지. 자녀의 사주 결을 정반대로 풀이하는 치명적 오류.
...
🔴 **신살별 정확 어휘 가이드 (의역 차단)**:
- **태극귀인(太極貴人)**: ❌ 금지 어휘 — "꿰뚫어 보는", "핵심을 파악", "예리한 통찰" (이건 현침살·문창귀인 의미와 혼동)
  ✅ 권장 어휘 — "음양 조화", "시작과 끝을 다스리는 큰 그릇", "한 분야를 끝까지 마무리", "종합·통합", "균형 잡힌 시야", "깊이 몰입하는 결"
- **현침살(懸針)**: ✅ "꿰뚫어 보는", "남이 보지 못하는 것을 보는 눈", "예리한 통찰"
- **문창귀인(文昌)**: ✅ "학문·문장의 별", "글과 말로 빛나는 결"
- **천을귀인(天乙)**: ✅ "어려움에 귀한 도움이 따르는 결", "결정적 인연"
- **양인살(羊刃)**: ✅ "강한 의지", "결단의 순간 빛나는 힘" (자원 톤)
- **화개살(華蓋)**: ✅ "예술·학문의 별", "자기만의 세계를 깊이 가꾸는"
- **도화살(桃花)**: ✅ "사람을 끌어당기는 매력", "사랑받는 결"
🔴 신살별 의미가 명리적으로 정확해야 합니다. 다른 신살의 의미를 임의로 차용 X.
...
- **합·충 모두 없음**: ❌ "드러나지 않는다" 단순 단정 금지. ✅ "사주 안에서 큰 합·충 자리가 도드라지지 않아, 자녀의 결이 **비교적 안정된 흐름** 속에서 자기 페이스로 자라나는 사주입니다. 급격한 변화나 강한 자극보다는 **꾸준하고 일관된 환경**이 자녀의 결을 가장 잘 살립니다." 톤으로 안정형 사주의 의미를 적극적으로 풀이 (단순 부정형 X).
...
- ✅ 권장 패턴: "${d.childName}**에게 평생 가까이 두면 빛나는** 한 결은 불의 결(火)입니다. **환한 빛, 따뜻한 사람들, 활기찬 활동**이 자녀의 결을 빛내줍니다." 같이 **무엇을 가까이 둘지 구체 예시 1줄 포함**.
- ✅ **오행별 구체 행동 예시 (필수 1~2개 본문에 명시)**:
...
- ✅ 권장 본문 모범 예시:
  "${d.childName}${d.childGender === '남' ? '군' : '양'}이 자라면서 친구 관계가 본격화될 시기가 오면, 부모님께서 한 걸음 떨어져 지켜봐 주시면 좋을 결입니다. 앞으로 자녀는 스스로 친구 갈등을 풀어가는 방법을 배워가게 될 텐데, 그때 부모님의 은은한 믿음이 자녀의 결을 단단하게 만들어 줄 것입니다. ${d.childName}${d.childGender === '남' ? '군' : '양'}의 결은 부모님의 따뜻한 거리 속에서 자기만의 페이스로 자라날 것입니다."
...
🔴 **절대 금지 단어**: "상극", "相剋", "충(沖)", "양인살", "괴강", "공망", "칠살", "형(刑)", "흉(凶)", "억누르다", "짓누르다", "압박", "꺾다", "다치다", "위협", "위축", "약하게 만들다", "거슬리다", "무너뜨리다". 부정 뉘앙스 금지 — 모든 결을 따뜻하게 풀어쓸 것.
★ 본문 마지막은 **균형형 마무리 두 문장** 권장 — 자원(가르침이 뿌리가 됨) + 한계(지나치지 않을 때) 함께 표현:

[형식]
"[부모 호칭]의 [관계 키워드]이 자녀에게 [자원 메타포: 단단한 뿌리·따뜻한 빛·새 시야 등]가 되어주는 결입니다. 다만 [부드러운 한계 조건: 너무 빠르거나 강하게 닿지 않을 때 / 자녀의 페이스를 침범하지 않을 때 / 한 박자 늦춰 다가가실 때], ${d.childName}${d.childGender === '남' ? '군' : '양'}의 결이 자기 페이스로 단단하게 자라납니다."

[좋은 예 — 극(剋) 관계]
"아버님의 확고한 가르침이 자녀에게 단단한 뿌리가 되어주는 결입니다. 다만 너무 빠르거나 강하게 닿지 않을 때, ${d.childName}양의 결이 자기 페이스로 단단하게 자라납니다."
