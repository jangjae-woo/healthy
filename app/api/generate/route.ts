...
1. **단락 1 — 격국 한 호흡** (1~2 문장, 약 90~110자): 자녀의 격국이 어떤 결인지. "**[격국 이름]([한자])**" 형식으로 한자병기 1회. 예: "${d.childName}${d.childGender === '남' ? '군' : '양'}는 [격국명]을 타고난 자녀입니다 — [한 줄 의미]."
2. **단락 2 — 일상에서 보이는 결** (2~3 문장, 약 110~140자): 격국의 본질이 자녀의 일상에서 어떻게 드러나는지 구체 장면 1~2개. (격국 = 자녀의 인생 큰 그림이자 평생 결의 방향)
3. **단락 3 — 인생의 큰 흐름과 자리** (1~2 문장, 약 80~110자): 자녀가 자라며 어떤 자리·환경·역할에서 가장 자기 결을 펼치는지 큰 그림 한 줄. 본질 인정 톤.
...
본문 구조 (반드시 이 흐름, 220~280자, 2~3 문단):
1. **단락 1 — 일주 캐릭터 한 호흡**: 자녀의 60갑자 일주를 자연·동물·풍경 비유로 한 줄 그려줄 것. 일간(천간) + 일지(지지)의 결합이 만드는 고유 캐릭터를 풀이. 예: "계묘(癸卯) — 깨끗한 물가에서 뛰노는 영리한 토끼", "갑진(甲辰) — 너른 들의 큰 나무".
2. **단락 2 — 일상에서 보이는 결**: 위 캐릭터가 자녀의 일상 모습으로 어떻게 드러나는지 1~2 장면.
3. **단락 3 — 본질 인정 마무리**: "이는 ${d.childName}${d.childGender === '남' ? '군' : '양'}의 타고난 일주의 결입니다" 톤.
...
🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **용신·기신 (PRIMARY)**. 이 페이지가 용신·기신의 **유일한 정의·풀이 슬롯**. 다른 페이지(개운법·절대 하면 안 학습·절대 하면 안 5가지 등)에서는 용신·기신 본질 풀이 절대 X — 한 줄 참조 또는 적용만 가능.

🔴 **부모 행동 경고의 PRIMARY 슬롯**: "단호함·비판·냉정·강요" 같은 부모 행동 경고는 **이 페이지에서만** (단락 2 — 기신 살핌). 다른 페이지에서 같은 어휘 경고 X.

★★★ **반드시 다음 결정론 시드도 함께 사용** (Phase 후속 — 용신·기신 통합):
- ${childGisinSeed}

본문 구조 (반드시 이 흐름, 380~480자, 3 단락):

**[단락 1 — 용신: 채울 결]** (약 130~170자)
이 자녀가 평생 채워가면 빛나는 한 결이 무엇인지 풀이. 자녀 일간 오행에 맞는 *구체 행동 1~2개* 반드시 명시. "용신" 한자 단어 본문 노출 금지 — "평생 가까이 두면 빛나는 한 결"로.

**[단락 2 — 기신: 살펴줄 결]** (약 130~170자)
"**기신(忌神)**" 한자병기 1회로 시작. 기신 = 자녀의 용신을 극(剋)하는 오행, 즉 *과하면 자녀의 빛을 흐리는 결*. 자녀 기신 오행을 명시. 위 시드의 "주의" 한 줄을 풀어, 그 오행이 *과하게 작용할 때* 자녀가 어떻게 흐려질 수 있는지 일상 장면 1~2개.

**[단락 3 — 양면 균형 마무리]** (약 80~110자)
...
🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **학당·문창·태극 신살 작용 (보조) + 인성·관성 강도 (보조)**. 신살 정의 절대 X (PRIMARY는 "### 타고난 귀인"). 인성·관성 정의 절대 X (PRIMARY는 "### 다섯 색깔의 결"). 식상 풀이는 한 줄만 (PRIMARY는 "### 다섯 색깔의 결" 식상 단락). 이 페이지는 **학습 자리에서의 작용·장면**만.

본문 구조 (반드시 이 흐름, 280~340자, 3 단락):
1. **단락 1 — 학습의 결 사주 근거** (약 90~110자): 자녀의 학습 채널을 *앞 페이지에서 본 학당·문창·태극* 작용 + 인성(받아들임)·관성(절제)·식상(표현) 강도로 풀이. 신살 정의 재서술 X — "(앞서 본) 학당의 결이 학습 자리에서 어떻게 작동하는가"만.
2. **단락 2 — 일상 학습 장면** (약 110~140자): 자녀가 가장 잘 흡수하는 *학습 방식 1~2개* 구체. 책상에서 깊이 vs 움직이며 익힘 / 듣고 따라가기 vs 직접 해보기 / 1:1 대화형 vs 자기주도. 사주 근거에서 도출.
3. **단락 3 — 학습 페이스 한 줄** (약 60~80자): 자녀의 학습 결이 가장 자연스럽게 자라는 페이스 한 문장.
...
본문 구조 (반드시 이 흐름, 240~300자, 2~3 단락):
1. **단락 1 — 자녀 일간 결을 거스르는 학습 방식 사주 근거** (약 90~110자): 자녀의 *일간 오행*이 학습에서 어떤 방식과 부딪히는지 짧게 풀이. 일간 정의 X — "${d.childName}${d.childGender === '남' ? '군' : '양'}의 일간 결은 [오행]이라 [학습 특화 부담 1줄]" 톤. 오행별 학습 부담 매핑:
   ...
   추가로 *식상이 강한* 자녀라면 "자기 표현을 막는 대본·정답 단순 암기" 부담 / *관성이 약한* 자녀라면 "강제 시간표·외부 통제 학습" 부담 한 줄 곁들이기.
2. **단락 2 — 부모가 피해야 할 구체 학습 행동** (약 100~130자): 부모가 무의식적으로 하기 쉬운 *학습 특화 역효과 행동 2~3가지* 구체. **반드시 위 학습 특화 어휘 풀에서만 선택** ("다른 자녀와 비교 / 결과만 평가 / 빠른 답 강요 / 한 번에 여러 과목 / 자녀 페이스 무시한 시간표 / 자기 표현 막기" 중 2-3개). "단호함·비판·냉정" 같은 일반 부모 행동 경고 어휘 X.
...
본문 구조 (한 단락, 240~300자, 2~3 문장):
1. **첫 문장 — 관계 형성 메커니즘 명시**: "${d.childName}${d.childGender === '남' ? '군' : '양'}은 *${friendS.dominant}* 결로 사람과 첫 인연을 맺습니다." 형식. **"관계 형성 메커니즘"** 어휘 본문 안에 자연스럽게 1회 노출.
2. **둘째 문장 — 사주 근거**: 자녀의 *일간 + 비겁·식상의 결*이 인연을 어떻게 끌어당기는지 한 줄 (예: "비겁이 단단해 같은 결의 친구를 자연스럽게 알아보는 자리").
3. **셋째 문장 — 부모 관찰 포인트**: 부모가 살피는 한 가지는 "**관계의 결이 자라는 환경**" 톤. 친구 *수*가 아니라 자녀의 결이 *어떤 환경에서 자기 인연을 끌어당기는지* 살펴주시면 좋다는 마무리.
...
본문은 한 단락(2~3줄). 흐름은 다음과 같이 구성:
1. **첫 문장 — 자녀의 갈등 수용 결**: "${d.childName}${d.childGender === '남' ? '군' : '양'}은 갈등이 생기면 *[일지 결에 따른 회복 패턴 한 줄]*로 받아들이는 결입니다." 일지의 합·충 관점에서 회복 결을 한 줄 (사주 용어는 "결"로 풀어쓰기).
2. **둘째 문장 — 권장 거리의 이유**: 위 회복 결 때문에 부모님께서 ${friendDist.label.split(" — ")[0]} 거리를 잡으시면 자녀의 회복이 자연스럽다는 흐름.
3. **셋째 문장 — 부모 가이드**: 자녀의 회복 결을 *살펴주시는* 톤으로 한 줄 마무리.
...
🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **12운성 + 일간·일지 신체 리듬 (PRIMARY)**. 12운성 본질·생체 리듬 메커니즘은 이 페이지에서 정의 가능. 다음 페이지("### 자녀에게 좋은 시간·환경")에서는 12운성 한 줄 참조만 — 정의 X.
...
본문 구조 (반드시 이 흐름, 280~340자, 2~3 단락):
1. **단락 1 — 자녀의 생체 리듬 결 + 사주 근거** (약 110~140자): "${d.childName}${d.childGender === '남' ? '군' : '양'}의 *생체 리듬*은 ~한 사주 결로 흐릅니다 (예: 인성·수가 강해 깊은 호흡 결 / 토 결이 약해 안정이 채워지는 시간 필요 / 화·목이 활발해 움직임이 먼저 풀려야 하는 결). 3채널 중 ${lifestyleTop.name}이 가장 자연스러운 결입니다." 형식. *일간·일지가 신체 리듬에 어떻게 작용하는지 한 줄* 본문에 분명히 명시.
2. **단락 2 — 리듬이 자연스럽게 흐르는 조건** (약 110~140자): TOP1·BOTTOM 채널이 *어떤 조건*에서 자연스럽게 흐르는지 구체 장면 1~2개. 카드 desc를 반복하지 말고, *리듬이 풀리는 조건* 중심.
3. **단락 3 — 부모의 한 줄 마무리** (약 60~90자): 자녀의 리듬 결을 *헤아리는* 부모 톤으로 마무리.
...
🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **용신 비보 5채널 (색·방위·음식·환경·시간) — 보조 슬롯**. 용신 본질·정의 풀이는 PRIMARY 슬롯("### 평생 빛나는 결 — 채움과 살핌 (용신·기신)")에서 끝났음. 이 페이지는 **용신 자체 정의·뜻 풀이 절대 X** — 한 줄 참조("자녀의 용신은 [오행]")만 + 비보 채널 적용에 집중.

★★★ **반드시 다음 결정론 시드만 사용** (Phase 3 신규):
- ${childGaeunSeed}

본문 구조 (반드시 이 흐름, 280~340자, 3 단락):
1. **단락 1 — 개운법의 의미 + 용신 한 줄 참조** (1~2 문장, 약 80~110자): "**개운법(改運法)**" 한자병기 1회로 시작. 개운법 = 자녀의 (앞서 본) *용신*에 맞춘 *오행 비보(裨補)*임을 한 줄로 압축. 용신 정의·본질 풀이 절대 X — "(앞서 본) 자녀의 용신 [오행]에 맞춰…" 톤 한 줄만.
2. **단락 2 — 일상에서 적용하는 법** (3~4 문장, 약 140~180자): 위 시드의 색·방위·음식·환경·시간대 중 **가장 와 닿는 2~3개**를 골라 자녀의 일상에 어떻게 적용할지 구체 장면. 예: "방 한쪽에 [색]의 작은 소품, 식탁에 [음식] 자주 올리기, [시간대]에 [환경] 활동하기"
3. **단락 3 — 부모의 살림 한 줄** (1~2 문장, 약 60~90자): 위 비보를 *자연스럽게 자녀 일상에 녹여* 자녀의 빛이 자라는 환경을 만들어주는 결로 마무리. (강제·교조 X — 부드러운 살림 톤)
...
🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **일주(일간·일지) 시간 호흡 톤 (PRIMARY 보조)**. 12운성 본질·신체 리듬 메커니즘 풀이 X — 한 줄 참조만 (PRIMARY는 "### 잠자리·식습관 안정 조건"). 본 페이지는 *시간대 호흡* 중심.
...
본문 구조 (반드시 이 흐름, 240~300자, 2~3 단락):
1. **단락 1 — 자녀의 호흡이 풀리는 시간 결** (1~2 문장, 약 80~110자): "${d.childName}${d.childGender === '남' ? '군' : '양'}의 호흡은 *[시간 결]*에서 가장 자연스럽게 풀립니다." 형식. 일주 기반 일상 호흡 강조 — 시드의 활기/가벼운 시간 표현 인용.
2. **단락 2 — 일상 호흡이 자연스럽게 흐르는 자리** (2~3 문장, 약 110~140자): 시드의 잠·학습·야외 가이드 3가지를 *호흡 흐름* 관점에서 풀이. "이 시간대는 자녀의 호흡이 ~한 결로 흐른다" 톤.
3. **단락 3 — 자녀의 일상 호흡 마무리** (1~2 문장, 약 60~90자): 자녀가 자신의 시간 결에 맞는 호흡으로 살아갈 때 가장 자기답게 자라는 결로 마무리.
...
🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **카드 5장이 사주 근거를 담당 (본문 = 일상 행동·말투 톤만)**. 본문에서 기신·용신·일간·십성 풀이 일체 X — 카드와 중복. "단호함·비판·냉정" 같은 Ch1 용신·기신 페이지와 중복되는 부모 행동 경고 어휘 본문 사용 X (카드 안에서만 가능). 본문은 *일상 행동 압축 한 줄*만.
...
🔴 **본문 구조** (반드시 이 흐름):
1. **시기 소개 한 줄**: "${d.childName}${d.childGender === '남' ? '군' : '양'}에게는 ${childCrisis.ageRange}이 ${childCrisis.source === "daeun" ? "*대운(大運) 변환점*과 맞물려 결이 자연스럽게 변하는 시기" : "결이 자연스럽게 변하는 시기 (대운 변환점 인근)"}로 다가옵니다."
2. **신호 3~4가지 (일상 장면)**: 자녀의 일간 오행에 맞는 구체 신호 짧게 나열.
3. **부모별 다가가는 길** (각 부모 입력에 따라 분기):
...
본문은 한 단락(2~3줄). 차트의 **가장 비슷한 비중의 결: ${momCompare?.similar.kor ?? "—"}** 과 **가장 비중이 다른 결: ${momCompare?.different.kor ?? "—"}** 이 일상에서 어떻게 드러나는지 *분포 관점*에서 풀어쓰기.
...
본문 구조 (반드시 이 흐름, 240~300자, 2~3 단락):
1. **단락 1 — 일지의 의미** (1~2 문장, 약 80~110자): "**일지(日支)**는 사주 4기둥 중 *일상의 결이 자리하는 자리*"임을 풀이. 위 일지 관계의 종류(육합·육충·형·해·파·비화·기타)를 한자병기 1회로 명시.
2. **단락 2 — 어디서 부드럽게 / 어디서 부딪히는가** (2~3 문장, 약 110~140자): 위 시드의 풀이 한 줄을 기반으로, 어머니와 자녀의 일상이 *식탁·취침·요리·정돈* 영역에서 *어디서 부드럽게 만나고 어디서 부딪히는지* 구체 장면 1~2개. (예: 육합 → "아침 식사·저녁 정돈에서 호흡이 자연스럽게 맞아요", 육충 → "취침 시간·식탁 정돈에서 결이 부딪힐 수 있어요")
3. **단락 3 — 관계의 자원** (1~2 문장, 약 60~90자): 어떤 관계여도 *그 자체로 의미*가 있음. 흔들림이 있다면 *자녀를 자라게 하는 자극*이라는 톤.
...
본문 구조 (반드시 이 흐름, 280~340자, 3 단락):
1. **단락 1 — 부모 십성의 의미 (PRIMARY 정의)** (1~2 문장, 약 90~110자): "자녀의 일간을 기준으로, ${d.momName}님(어머니)의 일간은 자녀에게 **${momParentSipseong?.sipseong ?? "—"}(${momParentSipseong?.hanja ?? ""})**의 결로 작용하십니다."로 시작. 십성 한자병기 1회. 부모 십성이 *왜 이런 결로 정의되는지* 한 줄 정의 풀이.
2. **단락 2 — 어머니가 자녀에게 어떤 결로 작용하나** (2~3 문장, 약 110~140자): 위 시드의 *meaning*과 *oneLiner*를 자연스럽게 풀어, 어머니가 자녀의 일상에서 어떤 결로 다가가는지 구체 장면 1~2개. (예: 인성 → "받아주고 길러주는 결", 식상 → "표현·창의 끌어내는 결")
3. **단락 3 — 어머니 결의 자원과 살핌** (1~2 문장, 약 80~110자): 그 결의 *자원*과 *과하면 살펴줄 점*을 한 줄로 균형. (예: 관성 → "단정한 모범의 결이 자녀를 바로 세워주되, 너무 강하면 부담이 될 수 있어요")
...
🔴 **본문 구조 (사용자 정책 — 풀이 풍부화)** — 반드시 이 흐름, **400~500자, 4 단락**:

1. **단락 1 — 함께 빛나는 별의 의미** (약 90~110자): "가족이 *공유하는 신살(神煞)*은 두 사주에 *같은 별이 자리*하는 경우입니다." 한자병기 1회. *함께 빛나는 별*이란 표현 강조. (정의 풀이 X — 작용·인연 톤만)

2. **단락 2 — 두 분의 신살 *각자 명시*** (약 110~140자): **자녀와 어머님 각자의 신살을 한자병기로 모두 명시**. 형식: "자녀(${d.childName}${d.childGender === '남' ? '군' : '양'})의 신살: **${(sajuChild.sinsal ?? []).join('·') || "—"}** — [엄마 곁에서 ~한 결로 깨어나는 작용 한 줄]. 어머님의 신살: **${(sajuMom?.sinsal ?? []).join('·') || "—"}** — [자녀 곁에서 ~한 결로 펼쳐지는 작용 한 줄]." (정의 X, 엄마 영역 작용·장면만)

3. **단락 3 — 공유 X / 공유 O의 의미** (약 100~140자):
${(momSharedSinsal?.shared.length ?? 0) > 0
  ? `**공유 신살(${momSharedSinsal?.shared.join("·")})의 의미**: 두 분이 같은 별을 가진다는 것은 *한 차원에서 깊이 닮은* 가족이라는 뜻. 일상 장면 1~2개로 풀이 (예: 천을귀인 공유 → "어려운 순간 서로가 결정적 인연이 되어주는", 문창 공유 → "함께 책·이야기에 깊이 빠지는").`
  : `**공유 신살이 없다는 것의 의미**: 두 분이 *서로 다른 차원에서 만나는* 가족 — 한 명이 가진 별이 다른 명에게 *없는 결*을 채워주는 *교환의 인연*입니다. 즉 *닮음*보다 *다름의 풍요*로 자라는 가족.`
}

4. **단락 4 — *상호 작용 구체 장면*** (약 100~130자): 두 분의 신살이 *일상에서 어떻게 상호 작용*하는지 구체 장면 1~2개. ${(momSharedSinsal?.shared.length ?? 0) > 0 ? `공유 신살을 함께 펼치는 장면.` : `자녀의 신살(예: 문창귀인 = 학문·글)이 빛날 때 어머님의 신살(○○)이 ○○해주고, 어머님의 신살이 펼쳐질 때 자녀의 신살이 ○○하는 식의 *서로 다른 별의 상호 작용* 장면.`}
...
본문 구조 (사용자 정책 — 풀이 풍부화) — 반드시 이 흐름, **3 단락, 약 250~300자**:

1. **단락 1 — 명리 근거 명시 (오행 흐름)** (약 80~100자): "어머님의 사주에 **[엄마 강 오행]의 결**이 강해서 자녀의 부족한 **[자녀 약 오행]의 결**을 자연스럽게 *흘려주십니다*." 형식. *오행 흐름*이 어떻게 일어나는지 사주 근거를 명확히.

2. **단락 2 — 일상 구체 장면** (약 100~130자): 어머님께서 자녀에게 그 결을 *어떤 일상 장면*에서 채워주시는지 1~2개. 예: "자녀가 새 도전을 망설일 때, 어머님의 활기찬 결이 *목의 봄결*로 작용해 산책·식물·새 환경을 자연스럽게 제안하십니다." 구체 장면 강제.

3. **단락 3 — 부모 행동 가이드 한 줄** (약 60~80자): "어머님께서 *의식 안 하셔도 자연스럽게 흐르는* 결입니다. 인위적 노력 X — 어머님 본질 그대로 곁에 계실 때 자녀의 결이 채워집니다." 톤.
...
🔴 **본문 풍부화 (사용자 정책)** — 카드 3개 이후 *본문 단락* 3 단락 추가, **약 250~300자**:

1. **단락 1 — 빛나는 순간의 명리 근거** (약 80~100자): "두 분이 가장 자연스럽게 빛나는 순간은 어머님의 **[엄마 강 십성/오행]**과 자녀의 **[자녀 강 십성/오행]**이 *동시 활성화*되는 자리에서 옵니다." 형식. *왜 그 순간 빛나는지* 명리 근거 한 줄.

2. **단락 2 — 빛나는 일상 순간 1~2개** (약 100~130자): 위 카드 중 1~2개를 골라 *주말·평일 어떤 일상 순간*에서 두 분의 결이 가장 자연스럽게 빛나는지 구체화. 예: "주말 오후 베이킹·미술 활동에서 두 분의 결이 가장 환히 빛나는 순간 — 자녀가 환히 웃는 표정이 자주 보이는 자리입니다."

3. **단락 3 — 활용 가이드 한 줄** (약 60~80자): "이 빛나는 순간을 *주 1회 의식적으로* 마련해 주실 때 자녀의 자존감이 자랍니다." 형식.
...
🔴 **본문 풍부화 (사용자 정책)** — 카드 2~3개 이후 *본문 단락* 3 단락 추가, **약 280~340자**:

1. **단락 1 — 반복되는 자리의 명리 근거** (약 90~110자): "어머님과 자녀가 같은 패턴으로 부딪히는 자리는 어머님의 **[엄마 강 십성]**과 자녀의 **[자녀 강 십성]**이 *서로 다른 차원*에서 작용하기 때문입니다." 형식. *왜 같은 패턴이 반복되는지* 명리 근거 한 줄.

2. **단락 2 — 반복 패턴이 발화하는 장면** (약 120~150자): 위 카드 중 가장 빈번한 갈등이 *어떤 일상 장면*에서 같은 패턴으로 반복되는지 구체화 (정서 영역 — 식탁·취침·말투·기분의 결). 예: "자녀가 정해진 일정을 어기거나 즉흥적으로 행동할 때, 어머님께서 즉각 제지하시면 자녀가 더 강하게 반발 — 일주일에 2~3회 같은 자리에서 부딪히는 패턴."

3. **단락 3 — 처치 행동의 *왜 효과 있는지*** (약 70~90자): "5분 기다리시면 자녀의 식상 결이 *자기 조절*로 자연 전환됩니다 — 강제로 다그치지 않을 때 자녀의 결이 더 단단해지는 자리입니다." 처치 행동의 *명리적 효과* 한 줄.
...
🔴 **본문 풍부화 (사용자 정책)** — 선물 카드 이후 *본문 단락* 3 단락 추가, **약 200~260자**:

1. **단락 1 — 명리 근거 명시** (약 70~90자): "어머님의 *일간 [한자]* 이 자녀의 *일간 [한자]* 에게 [생/극/비화/합] 의 자리이므로, *어머님 결의 어느 한 부분*을 한 박자 조절하시면 자녀에게 가장 도움이 됩니다." 형식.

2. **단락 2 — 어떤 결을 한 박자 늦추면 좋은가** (약 70~90자): "어머님의 [구체 결 — 따뜻함·살핌·기다림 등] 이 *한 박자 빨리* 닿으면 자녀가 자기 결을 펼칠 자리를 잃을 수 있습니다." 형식.

3. **단락 3 — 조절 후 변화 한 줄** (약 60~80자): "한 박자 늦춰 다가가실 때, 자녀가 *자기 박자*로 단단해집니다." 형식의 *조절 효과* 한 줄.
...
본문은 한 단락(2~3줄). 차트의 **가장 비슷한 비중의 결: ${dadCompare?.similar.kor ?? "—"}** 과 **가장 비중이 다른 결: ${dadCompare?.different.kor ?? "—"}** 을 *방향성의 차이* 관점으로 풀어쓰기. 아버님 특유의 "기둥의 결"(안정·방향성) 관점.
...
본문 구조 (240~300자, 2~3 단락):
1. **단락 1 — 두 분의 일지 관계 핵심** (1~2 문장): 일지 정의 재서술 X — 바로 "아버님과 자녀의 일지가 [관계명]으로 만난다"는 사실 진입. 한자 1회만.
2. **단락 2 — 함께하는 활동에서 자라나는 결** (2~3 문장): 위 시드의 풀이를 기반으로, *아빠 영역(야외·운동·만들기·주말 활동)* 에서 두 분이 함께할 때 *어떤 결이 자라나는지* 구체 장면 1~2개.
3. **단락 3 — 관계의 자원** (1~2 문장): 어떤 관계여도 의미가 있고, 흔들림이 있다면 자녀를 자라게 하는 자극이라는 톤. 단, 엄마 일지 페이지의 결론 문장과 다른 표현으로.
...
본문 구조 (280~340자, 3 단락):
1. **단락 1 — 사실 진입 (모범의 결)** (1~2 문장): 십성 정의 재서술 X — "아버님의 일간은 자녀에게 [십성]의 *모범 결*로 작용하십니다." 톤. 한자 1회만.
2. **단락 2 — 아빠 영역 모범 장면** (2~3 문장): 위 시드의 풀이를 풀어, 아버지가 자녀에게 *어떤 모범의 결*을 보여주시는지 **아빠 영역(야외·결단·만들기)** 구체 장면 1~2개. (예: 정관 → "약속·기준을 단단히 지키시는 모범", 식상 → "새로운 일에 먼저 손대시는 모범")
3. **단락 3 — 아버지 결의 자원과 살핌** (1~2 문장): 그 모범 결의 자원과 과하면 살펴줄 점을 한 줄로 균형. 엄마 페이지의 결론 표현과 다른 어휘로.
...
1. **단락 1 — 함께 빛나는 별이 깨어나는 자리** (약 90~110자): "가족이 *공유하는 신살(神煞)*은 두 사주에 *같은 별이 자리*하는 경우입니다." 한자병기 1회. 정의 X — *어떤 활동에서 깨어나는지* 톤.

2. **단락 2 — 두 분의 신살 *각자 명시*** (약 110~140자): **자녀와 아버님 각자의 신살을 한자병기로 모두 명시**. 형식: "자녀(${d.childName}${d.childGender === '남' ? '군' : '양'})의 신살: **${(sajuChild.sinsal ?? []).join('·') || "—"}** — [아빠와 함께 ~한 활동에서 깨어나는 작용 한 줄]. 아버님의 신살: **${(sajuDad?.sinsal ?? []).join('·') || "—"}** — [자녀와 함께 ~한 활동에서 펼쳐지는 작용 한 줄]." (정의 X, 활동 영역 한정)

3. **단락 3 — 공유 X / 공유 O의 의미** (약 100~140자):
${(dadSharedSinsal?.shared.length ?? 0) > 0
  ? `**공유 신살(${dadSharedSinsal?.shared.join("·")})의 의미**: 두 분이 같은 별을 가진다는 것은 *한 차원에서 깊이 닮은* 가족이라는 뜻. 일상 장면 1~2개로 풀이.`
  : `**공유 신살이 없다는 것의 의미**: 두 분이 *서로 다른 차원에서 만나는* 가족 — 한 명이 가진 별이 다른 명에게 *없는 결*을 채워주는 *교환의 인연*입니다. 즉 *닮음*보다 *다름의 풍요*로 자라는 가족.`
}

4. **단락 4 — *상호 작용 구체 장면*** (약 100~130자): 두 분의 신살이 *일상에서 어떻게 상호 작용*하는지 구체 장면 1~2개. ${(dadSharedSinsal?.shared.length ?? 0) > 0 ? `공유 신살을 함께 펼치는 장면.` : `자녀의 신살이 빛날 때 아버님의 신살이 ○○해주고, 아버님의 신살이 펼쳐질 때 자녀의 신살이 ○○하는 식의 *서로 다른 별의 상호 작용* 장면.`}
...
1. **단락 1 — 명리 근거 명시 (단단함과 방향)** (약 80~100자): "아버님의 사주에 **[아빠 강 오행]의 결**이 단단해서 자녀에게 **[자녀 약 오행]의 결**을 *방향성*으로 더해주십니다." 형식. *왜 그 결이 단단함·방향으로 작용하는지* 사주 근거를 명확히.

2. **단락 2 — 일상 구체 장면** (약 100~130자): 아버님께서 자녀에게 그 결을 *어떤 일상 장면*에서 단단함·방향으로 더해주시는지 1~2개. 아빠 특유의 *기둥의 결*(안정·방향성) 관점으로 구체화.

3. **단락 3 — 부모 행동 가이드 한 줄** (약 60~80자): "아버님께서 *의식 안 하셔도 자연스럽게 흐르는* 결입니다. 인위적 노력 X — 아버님 본질 그대로 곁에 계실 때 자녀의 결이 채워집니다." 톤.
...
1. **단락 1 — 함께 도전이 자연스러운 자리의 명리 근거** (약 80~100자): "두 분의 도전이 가장 자연스러운 순간은 아버님의 **[아빠 강 십성/오행]**과 자녀의 **[자녀 강 십성/오행]**이 *동시 활성화*되는 자리에서 옵니다." 형식.

2. **단락 2 — 함께 도전하는 일상 순간 1~2개** (약 100~130자): 위 카드 중 1~2개를 골라 *주말·야외·만들기* 같은 *도전 활동* 중 어디서 시너지가 가장 잘 일어나는지 구체화. 아빠 특유의 *기둥의 결*(안정·방향성) 관점으로.

3. **단락 3 — 활용 가이드 한 줄** (약 60~80자): "이 도전 시너지는 *주 1회 의식적으로* 마련해 주실 때 자녀의 자존감이 자랍니다." 형식.
...
1. **단락 1 — 결단·방향 충돌의 명리 근거** (약 90~110자): "두 분의 결단·방향이 부딪히는 자리는 아버님의 **[아빠 강 십성]**과 자녀의 **[자녀 강 십성]**이 *다른 방향*을 향하기 때문입니다." 형식.

2. **단락 2 — 방향이 부딪히는 장면 구체화** (약 120~150자): 위 카드 중 가장 빈번한 갈등이 *어떤 결단·선택 장면*에서 시작·확대되는지 구체화 (진로·활동·도전 영역). 아빠 특유의 *기둥의 결*·*방향성* 관점.

3. **단락 3 — 처치 행동의 *왜 효과 있는지*** (약 70~90자): 처치 행동의 *명리적 효과* 한 줄. "강제로 다그치지 않을 때 자녀의 결이 더 단단해지는 자리입니다" 톤.
...
1. **단락 1 — 명리 근거 명시** (약 70~90자): "아버님의 *일간 [한자]* 이 자녀의 *일간 [한자]* 에게 [생/극/비화/합] 의 자리이므로, *아버님 결의 어느 한 부분*을 한 박자 늦추시면 자녀에게 가장 도움이 됩니다." 형식.

2. **단락 2 — 어떤 결을 한 박자 늦추면 좋은가** (약 70~90자): "아버님의 [구체 결 — 단호함·결정·방향 제시 등] 이 *한 박자 빨리* 닿으면 자녀가 자기 박자를 잃을 수 있습니다." 형식.

3. **단락 3 — 조절 후 변화 한 줄** (약 60~80자): "한 박자 늦춰 다가가실 때, 자녀가 *자기 박자*로 단단해집니다." 형식.
...
본문 구조 (반드시 이 흐름, 240~300자, 2~3 문단):
1. **단락 1 — 시기 소개**: "${d.childName}${d.childGender === '남' ? '군' : '양'}이 자라며 진로의 결을 정해갈 시기는 만 14~17세 무렵입니다." 형식.
2. **단락 2 — 사주 흐름**: 위 대운 변환점이 자녀의 결을 어떻게 변화시키는지 한 줄. 진로 적합 영역(${jobTop1.name}) 의 결이 그 시기에 어떻게 빛날 수 있는지.
3. **단락 3 — 부모 안내**: 진로 결정 시기 전·후로 부모님께서 어떤 시각·태도로 자녀를 바라보시면 좋은지 한 줄. 강제·단정 X.
