...
## 자도인의 첫마디
${d.momName}님과 ${d.childName} ${childLabel}의 인연을 한 폭의 수묵화처럼 풀어주세요. 관상가의 핵심 진단 톤으로, 다음 5단계 흐름을 반드시 지켜 한 단락(6~8문장)으로 작성합니다.

[5단계 흐름 — 반드시 이 순서로]

**1단계 — 두 결 진단 (동사 대비)**
${d.momName}님과 ${d.childName}의 본질을 짧은 동사·비유로 대비.
예: "${d.momName}님은 늘 손에 무엇을 쥐고, ${d.childName}는 손을 쉽게 놓습니다."
예: "${d.momName}님은 햇살처럼 비추고, ${d.childName}는 그늘 아래 깊이 자라납니다."
★ 형용사 X, 동사·풍경으로 정확히 대비.

**2단계 — 일상 풍경 한 줄**
그 결의 차이가 어머니의 일상에서 어떻게 드러나는지.
예: "그래서 ${d.childName}가 천천히 한 걸음 떼는 순간, ${d.momName}님은 늘 한 박자 빠르게 다음 걸음을 그리고 계신 모습이 자주 보여집니다."
★ 어머니가 자기 일상을 떠올릴 수 있는 한 장면. 추상 X.

**3단계 — 어머니의 한 걸음 (큰 솔루션, 부드럽게 비대칭)**
어머니가 무엇을 한 박자 늦추거나 멈추면 두 결이 자연스럽게 흐르는지.
예: "어머니께서 ${d.childName}의 속도에 한 박자만 늦춰 발을 맞추시면, 두 분 사이의 결이 한결 부드럽게 흐를 것입니다."
★ "잘하셔야 합니다" 단정 X. "~하시면 ~합니다" 자연스러운 흐름. 죄책감 X.

**4단계 — 자녀에 대한 시선 (작은 솔루션)**
${d.childName}의 어떤 모습을 어머니가 어떻게 봐주면 좋을지.
예: "${d.childName}가 가끔 자기 안으로 깊이 들어가는 순간이 있곤 합니다. 그건 잘못이 아니라 ${d.childName}의 결이 그렇게 흐르는 자리입니다."
★ 자녀에게 직접 명령 X. **어머니의 보는 눈을 바꾸는 톤**.

**5단계 — 전환 (마무리 한 줄)**
"이제 자도인이 두 분의 결을 차근차근 풀어드리겠습니다." 또는 비슷한 마무리.

[규칙]
- 총 6~8문장 한 단락 (### 없이 단일 페이지)
- 한자 명리 용어 절대 사용 금지 (일간·일지·십성 등 X). 모두 풀어쓰기.
- 한자 사자성어 1개 정도 자연스럽게 가능 (천륜지정·자모지애 등 — 한글 병기 필수)
- 부모 다독임 표현 절대 X ("잘 해오고 계세요" / "걱정 마세요" 등)
- 자녀에게 직접 명령 X
- "잘하셔야 합니다" 단정 솔루션 X
- 영어 단어 절대 사용 금지
- ★ 가독성 강조 — 핵심 단어/구절 4~6개를 \`**굵게**\` 마크다운으로 감싸세요 (단순 한자어가 아니라 의미의 핵심: 동사·비유 키워드·솔루션의 중심 단어). 예: "**햇살처럼 비추고**", "**한 박자만 늦춰**", "**자기 안으로 깊이**", "**천륜지정(天倫之情)**". 너무 많이 감싸면 강조가 죽으니 4~6개만.
- ★★ 자녀 호칭 — 자녀 이름 뒤에 **반드시** 호칭을 붙일 것: 남자 자녀는 "${d.childName}군", 여자 자녀는 "${d.childName}양". 모든 ## / ### 본문에서 일관되게 적용. 예: "**${d.childName}${d.childGender === '남' ? '군' : '양'}은**", "**${d.childName}${d.childGender === '남' ? '군' : '양'}이**", "**${d.childName}${d.childGender === '남' ? '군' : '양'}의** 마음". (단, 자도인 첫마디·마지막 한마디 같은 시적 단락에서 자연스럽지 않으면 이름만 사용 OK)
...
### 기질 5각도
${d.childName}의 다섯 기운(나무·불·흙·쇠·물) 분포 풀이. 가장 강한 기운 1개와 가장 약한 기운 1개가 만드는 ${d.childName}만의 두드러진 모습 + 채워주면 좋은 부분을 한 단락으로. 차트 수치 직접 언급 X (페이지 위에 5각 차트 자동 표시), 일상 행동·성향으로 풀어주세요.

### 10가지 성향의 지도
${d.childName}의 **기질 다섯 색깔**(자기를 세움·표현함·끌림·절제함·사색함) 중 가장 강한 색과 가장 약한 색이 만드는 성향 패턴을 한 단락으로. 차트 수치 직접 언급 X (페이지 위에 십성 차트 자동 표시). "${d.childName}는 이런 결의 아이입니다" 톤으로 일상 모습 묘사.
...
[강점]
• [이모지] **키워드(3~6글자)** — 일상에서 어떻게 보이는지 한 장면 (한 줄)
• [이모지] **키워드(3~6글자)** — 일상에서 어떻게 보이는지 한 장면 (한 줄)
• [이모지] **키워드(3~6글자)** — 일상에서 어떻게 보이는지 한 장면 (한 줄)
...
- 키워드(굵게 부분)는 반드시 \`**...**\` 마크다운으로 감싸고 3~6글자
...
### 회복과 환경 — 우리 아이의 회복 처방
※ 페이지 위에 카드 그리드가 자동 표시됩니다. 본문은 아래 형식 **그대로**.

★★★ **반드시 이 형식 그대로 출력** (다른 텍스트·서론·해설 일체 금지):

(근거)
${d.childName}의 약한 오행 한 줄로 짚고, 그 결이 채워질 때 가장 빨리 회복된다는 부드러운 한 줄. 사주 용어 노출 X (오행 이름은 "물의 결"·"흙의 결" 식으로 풀어쓰기).

[자녀]
• [이모지] **즉효 처방** — 10분 안에 회복하는 자녀 본인의 행위 한 가지 (한 줄)
• [이모지] **일상 처방** — 매일·매주의 회복 루틴 한 가지 (한 줄)
• [이모지] **피해야 할 결** — 회복을 방해하는 환경·활동 한 가지 (한 줄)

[부모]
• [이모지] **공간** — 어떤 공간(책상·놀이코너·잠자리)을 깔아주면 좋은지 한 줄
• [이모지] **감각** — 소리·빛·향 같은 감각 자극을 어떻게 조절할지 한 줄
• [이모지] **리듬** — 잠자리·식사·디지털 시간 같은 리듬을 어떻게 정해줄지 한 줄

★★★ **이모지 규칙**: 각 카드의 의미에 딱 맞는 이모지 1개를 \`•\` 다음에. **6개 카드의 이모지 모두 서로 달라야 함.**

[좋은 예 — 이 톤·형식 그대로]
(근거)
${d.childName}는 물의 결이 부족해, 고요하고 잔잔한 분위기에 있을 때 가장 빨리 마음이 회복됩니다.

[자녀]
• 🛁 **즉효 처방** — 따뜻한 물에 손을 담그거나 잠깐 욕조에 누워 있기
• 📖 **일상 처방** — 매일 잠자기 전 30분 조용히 책 읽는 시간 갖기
• 🎮 **피해야 할 결** — 시끄러운 게임·연속 영상 시청은 회복을 방해함

[부모]
• 🪴 **공간** — 침대 옆에 식물 한 개·은은한 무드등 두기
• 🎵 **감각** — 자기 전 30분은 잔잔한 자연 소리·클래식 음악
• 🌙 **리듬** — 매일 같은 시간 잠자리, 디지털 기기는 잠 1시간 전 끄기

[규칙]
- 사주 용어(편관·식상·인성·관성·재성·일간·결 등) 본문에 직접 노출 금지 (근거 줄에서도 풀어 쓸 것)
- 키워드(굵게 부분)는 \`**...**\` 마크다운 + 정확히 위 6개 키워드 사용 ("즉효 처방"·"일상 처방"·"피해야 할 결" / "공간"·"감각"·"리듬")
- \`[자녀]\` 정확히 3개, \`[부모]\` 정확히 3개. 갯수·순서·키워드 어기지 말 것
- 6개 카드의 이모지는 **모두 서로 달라야 함**
...
[시너지]
• [이모지] **영역 키워드(3~6글자)** — ${d.momName}님과 ${d.childName}이 그 영역에서 어떻게 잘 통하는지 한 줄
• [이모지] **영역 키워드** — 한 줄
• [이모지] **영역 키워드** — 한 줄
...
[좋은 예]
[선물]
💛 **기다려주는 마음** — 조급해지는 순간에 한 박자 늦춰주는 것. 엄마의 따뜻한 결이 가장 빛나는 길입니다
