...
구성: ① 결 단정 (50~70자) → ② 비겁 메커니즘 + 비견(比肩)·겁재(劫財) 분리 + 식상·신강신약·일주·신살 결합 풀이 (200~250자) → ③ 일상 학습 장면 묘사 (150~200자) → ④ 학습 환경 권고 (100~150자) → ⑤ 공식형 한 줄 — 마지막 줄에 "공부는 ○○ — 점검은 △△. 이게 ${cnh}의 학습 리듬이에요." 같은 X=Y 공식 형태. 비겁 0이면 "남의 결을 받아들이는 협력형 사주(無比劫)" 양면 풀이로 동일 시그너처 유지.

### 우리 아이만의 공부법
[메인: 인성 / 서브: 식상·신강신약·일주(일간 오행)·문창귀인·학당귀인]
[시그너처: 두 갈래 비교] — 본문 안에 "잘 들어오는 방식"과 "안 들어오는 방식" 두 갈래 명시 비교. 분량 500~600자.
구성: ① 결 단정 (60~80자) → ② 인성(印星) 메커니즘 + 정인(正印)·편인(偏印) 분리 + 식상·신강신약·일간 오행 결합 풀이 (180~220자) → ③ "잘 들어오는 방식" 단락 (구체 학습법 1~2개, 100~140자) → ④ "안 들어오는 방식" 단락 (반대 케이스 구체, 100~140자) → ⑤ 부모 한 줄 권고 (40~60자). 두 갈래는 줄바꿈으로 시각 분리. 인성 0이면 "직관·실전형 사주" 두 갈래 양면 풀이.
...
구성: 자도인이 ${cnh}의 일간을 자연 비유로 풀어내며 하루를 따라간다. ① 일간 오행 비유 한 줄 ("${cnh}의 일간은 화(火)예요. 한낮의 햇살처럼...") → ② 새벽~아침 시간대 ${cnh} 결 묘사 (60~80자) → ③ 정오~오후 시간대 (60~80자) → ④ 저녁~밤 시간대 (60~80자) → ⑤ 12운성·월지·시주 결합 풀이 한 단락 (100~140자) → ⑥ 학습 스케줄 권고 한 줄. 단락 사이 줄바꿈으로 시간 흐름 시각화. 5단락 템플릿 사용 X.

### 책상 앞 머릿속
[메인: 관성 / 서브: 인성·신강신약·토 오행·신살(괴강·천을귀인)]
[시그너처: 관찰자 시점 묘사] — 자도인이 자녀 옆에서 책상 앞 모습을 관찰하듯 영화의 한 장면처럼 묘사 → 그 행동의 사주적 의미 풀이. 분량 500~600자.
구성: ① 관찰 묘사 ("${cnh}이 책상 앞에 앉아 있어요. 펜을 잡기 전, 한 번 책을 들었다 놨다 합니다. 문제 하나를 풀고는 정답을 확인하고, 또 한 번 확인합니다." 같은 90~120자 장면) → ② 그 행동의 사주적 의미 — 관성(官星) 메커니즘 + 정관·편관 + 인성·신강신약·토 오행·신살 결합 (200~250자) → ③ "이 머릿속에 늘 함께 앉아있는 작은 질문" 묘사 (80~100자) → ④ 환경 권고 (자리·시간·루틴 3요소, 80~120자). 관성 0이면 "틀에 안 갇히는 자유로운 책상 앞" 관찰 시그너처 유지.`;
...
### 화났을 때 입을 닫을까, 폭발할까
[메인: 식상 / 서브: 비겁·신강신약·일주·신살]
[시그너처: 두 모드 묘사] — 자녀가 화났을 때 보이는 두 가지 모드(폭발 모드 / 닫힘 모드)를 각각 한 단락씩 묘사하고, 자녀 사주가 어느 쪽에 가까운지 결정. 분량 500~600자.
구성: ① "${cnh}은 화가 날 때 어떤 모드로 작동하나요" 도입 (60~80자) → ② **폭발 모드 묘사 단락** (식상 강·신강·양인살 케이스, 90~120자) → ③ **닫힘 모드 묘사 단락** (식상 약·신약·고란살 케이스, 90~120자) → ④ ${cnh}의 사주 결합으로 결정 — "${cnh}은 [폭발 모드/닫힘 모드/두 모드 사이]에 가까워요" + 그 근거 (식상·비겁·신강신약·일주·신살 결합, 150~180자) → ⑤ 부모 첫 대응 한 줄. 식상 0이면 "안으로 삼키는 결·말로 다 풀지 않는 신중한 사주" 두 모드 시그너처 유지.

### 아이 감정이 가라앉는 환경
[메인: 오행 / 서브: 일주·신강신약·기신 오행·12운성]
[시그너처: 처방전 리스트] — 자도인이 환경 처방전을 작성하듯 구체 항목 리스트로 출력. 분량 450~550자.
구성: ① 진단 한 줄 ("${cnh}의 결을 가장 빠르게 가라앉히는 환경은 ○○이에요." 자녀 강한 오행 비유) → ② 처방 도입 — 강한 오행과 약한 오행·기신 오행 결합 메커니즘 (140~180자) → ③ **처방 리스트 5항목**: 공간 / 색 / 소리 / 온도 / 접촉. 각 항목 한 줄씩 자녀 사주 데이터 기반 구체 권장 (예: "공간 — 노란빛이 도는 따뜻한 모서리, 너무 넓지 않은 곳" 같은 30~50자/항목, 총 150~250자) → ④ 일주·12운성 결합 한 줄 권고. 처방전 형식 = 부모가 즉시 적용 가능한 구체성. 일반론 절대 금지.

### 마음 열리는 칭찬
[메인: 인성 / 서브: 식상·일주·용신·천을귀인]
[시그너처: 칭찬 멘트 예시 — 대화체] — 자도인이 부모에게 칭찬 멘트를 직접 들려주는 대화체. 분량 450~550자.
구성: ① "이 아이의 마음을 여는 칭찬은 따로 있어요" 도입 (60~80자) → ② 인성 결합 메커니즘 — 정인(過程·태도형) vs 편인(직관·고유성형) + 식상·일주·용신 결합 (150~180자) → ③ **실제 칭찬 멘트 예시 2~3개** (각 인용부호로 30~50자씩, 자녀 사주 인자 기반. 예: "'그 부분 진짜 네 식대로 풀었네'라는 식으로 자녀 결을 인정해주세요" — 100~150자) → ④ 안 통하는 칭찬 짧게 (40~60자, "그냥 잘했어'는 ${cnh}에겐 와닿지 않아요") → ⑤ 한 줄 정리. 인용부호로 멘트 분명히 구분.

### 거짓말 했을 때
[메인: 일주 / 서브: 관성·인성·신살·합·충]
[시그너처: 가상 대화 + 사주 풀이] — 자녀와 부모의 짧은 가상 대화 1개 + 그 대화 뒤의 자녀 결 풀이 + 결을 짚는 대화법 권장. 분량 500~600자.
구성: ① 도입 한 줄 → ② **가상 대화 박스**: 부모가 거짓말을 발견했을 때 — "엄마: '○○ 했지?' / ${cnh}: '...'" 형식 2~3 turn (자녀 일주·관성 결합 반응 — 정관 강 = 입 다물고 시선 회피 / 편관 강 = 강하게 부정 등, 100~140자) → ③ 그 반응의 사주적 의미 풀이 — 일간·일지·관성·인성·신살·합충 결합 (180~220자) → ④ 결을 짚는 대화법 — "혼내지 말고 ○○로 짚으세요" 권장 한 단락 (100~140자) → ⑤ 마무리 한 줄. 대화 박스는 줄바꿈으로 시각 분리.

### 이 아이가 무너지는 자극
[메인: 기신 / 서브: 약한 오행·신강신약·신살·12운성]
[시그너처: 신호등 진단] — 부모가 미리 알아둘 신호 3가지를 단계별(노랑·주황·빨강)로 제시. 분량 500~600자.
구성: ① 도입 — "이 아이가 무너지는 자극은 따로 있어요" + 기신(忌神) 한자 (70~90자) → ② 기신 메커니즘 + 약한 오행·신강신약·신살 결합 풀이 (150~180자) → ③ **신호 3단계 박스**: 🟡 노랑 (살피기 시작 신호) → 🟠 주황 (개입 필요 신호) → 🔴 빨강 (반드시 환경 바꿔야 신호). 각 단계마다 자녀 사주 기반 구체 행동/상황 (각 50~70자, 총 150~220자) → ④ 부모 한 줄 권고 — "이 신호가 보이면 ○○하게 멈춰주세요". 부정형 표현 금지 — "살펴줄 결" 톤. 신호등 박스는 이모지로 시각 분리.`;
...
[시그너처: 시간 단위 진단 + 단계별 묘사] — 자녀가 새 친구에게 마음을 여는 데 걸리는 "구체 시간 단위"를 진단(예: "한 학기" "두 달" "한 주") + 단계별 마음 열림 과정 묘사. 분량 500~600자.
구성: ① 진단 한 줄 ("${cnh}이 새 친구에게 마음 여는 데는 보통 [한 학기/한 달/한 주] 정도 걸려요." 자녀 일주·인성·신강신약 결합으로 결정) → ② 일주 메커니즘 + 일간 음양·일지 십성·합·충 결합 풀이 (150~200자) → ③ **마음 열림 단계 묘사**: 1단계 (탐색기) → 2단계 (인정기) → 3단계 (마음 여는 시점). 각 단계 자녀 사주 기반 구체 행동 (총 150~200자) → ④ 학기 초·새 환경 부모 도움법 한 단락 (80~120자). 비겁 0이면 "마음 빨리 여는 협력형" 시그너처 유지.

### 리더 vs 짝꿍 vs 분위기 메이커
[메인: 비겁·식상·관성 / 서브: 일주·신살(괴강·양인)]
[시그너처: 3 유형 진단 + 자녀 비중 표시] — 3 유형(리더 / 짝꿍 / 분위기 메이커)을 짧게 정의하고, 자녀 사주에서 각 유형 비중을 % 가깝게 표현. 분량 500~600자.
구성: ① 도입 — "친구들 사이에서 ${cnh}이 어디에 가까운지" → ② **3 유형 짧은 정의** (각 한 줄, 비겁 = 리더 / 식상 = 분위기 메이커 / 관성 = 짝꿍·정리, 총 100~140자) → ③ ${cnh}의 사주 비중 진단 — "${cnh}은 [리더 60% + 분위기 메이커 40%] 같은 결" 식으로 두 유형 비중. 비겁·식상·관성 강도 + 괴강·양인살 결합 풀이 (200~250자) → ④ 자연스럽게 자리 잡는 친구 그룹 묘사 (80~120자) → ⑤ 부모 한 줄 권고. 단정 X — 두 유형 결합 형태로.

### 인생을 바꿀 친구는 따로 있다
[메인: 귀인 신살 / 서브: 일주·일간 오행·합·대운]
[시그너처: 인연의 단서 — 띠·계절·성향 명시] — 자녀의 사주 귀인 신살이 만나줄 친구의 구체 단서를 띠·계절·성향으로 짚어줌. 분량 500~600자.
구성: ① 도입 — "${cnh}의 인생을 바꿀 친구는 따로 있어요" → ② 귀인 신살 메커니즘 + 천을(天乙)·복성·문창·학당귀인 등 보유 시 친구 결 결정 풀이 (150~200자) → ③ **인연의 단서 박스**: 띠 (자녀 일지 삼합·육합 글자에 해당하는 띠 1~2개 명시) / 계절 (자녀 일간과 상생 관계 계절) / 성향 (자녀 일간 오행과 상생 = 결 키워줌, 상극 = 결 흔들지만 자라게 함) (총 150~200자) → ④ 일상 단서 — "이런 결의 친구를 만나면 알아보세요" 한 단락 (80~120자). 귀인 0이면 "스스로 어른의 결을 만나러 가는 자수성가형" 단서 시그너처 유지(친구가 인연이 아니라 환경이 인연).

### 친구의 결이 바뀌는 시기
[메인: 대운 / 서브: 일간 오행·신강신약·세운]
[시그너처: 타임라인 짚어주기] — 자녀 대운 흐름을 시간선으로 짚으며 변환점 1~2개를 구체 나이로 명시. 분량 500~600자.
구성: ① 도입 — "${cnh}의 친구 결은 평생 같지 않아요" → ② 대운 메커니즘 — 10년마다 결 바뀜 + 자녀 일간과 합·충 시기 결합 (150~180자) → ③ **타임라인 짚기**: ${cnh}의 대운 8개 중 친구 환경이 크게 바뀌는 변환점 1~2개를 "○세 무렵" + 그 시기 어떤 변화 (200~250자, 자녀 사주 데이터 기반 구체) → ④ 부모가 그 시기 어떻게 곁에 있을지 한 단락 (80~120자). 사춘기 표현 = "결이 변하는 시기". 단정 금지 — "큰 흐름은 ~한 시기에" 톤.

### 친구들 속에서 지치는 패턴
[메인: 신강신약 / 서브: 비겁·식상·일주·12운성]
[시그너처: 에너지 그래프 묘사] — 자녀의 친구 모임 전·중·후 에너지 변화를 그래프 묘사하듯 풀이. 분량 450~550자.
구성: ① 진단 — "${cnh}은 친구 모임에서 [길게 견디는/짧게 강하게/금방 지치는] 결" → ② 신강신약 메커니즘 + 비겁(자기 결 단단함) + 식상(외부 표현 에너지) + 12운성(약세=태·묘·절) 결합 풀이 (150~200자) → ③ **에너지 그래프 묘사** — 모임 전 → 모임 중 → 모임 직후 → 24시간 후. 각 시점 ${cnh}의 결 묘사 (4 시점 총 120~160자) → ④ 회복 환경 1~2개 구체 권장 (80~100자). 신약·12운성 약세는 "회복 결" 풀이로.`;
...
[시그너처: 분야 결 — 키워드 묶음 + 산문 풀이] — 자녀 사주에서 도출한 분야 키워드 3~5개를 묶음으로 노출 + 그 키워드들이 왜 자녀에게 맞는지 산문 풀이. 분량 500~600자.
구성: ① 도입 — "${cnh}이 진짜 빛날 분야는 사주 안에 이미 새겨져 있어요" → ② **분야 키워드 묶음 박스** (자녀 식상·재성·일간 오행 결합 → 키워드 3~5개. 예: "창작 / 표현 / 사람 / 무대 / 즉흥" 같은 결 키워드. 직업명 X — 결의 형태로) → ③ 식상·재성 결합 메커니즘 + 일간 오행별 분야 결(목=교육·창작 / 화=예술·발표 / 토=신뢰·관리 / 금=정밀·결단 / 수=연구·지혜) + 관성·신강신약 결합 (200~250자) → ④ 그 분야에서 ${cnh}이 강해질 한 가지 결 한 단락 (100~120자). 직업명 절대 단정 X.

### 아이만의 무기
[메인: 일주 / 서브: 신살·일간 오행·신강신약·격국 흐름]
[시그너처: 무기 1개 명명 + 빛나는 순간 묘사] — 자녀 일주에서 도출한 "단 하나의 무기"를 자도인이 명명해주고, 그 무기가 빛나는 구체 순간을 영화의 한 장면처럼 묘사. 분량 450~550자.
구성: ① 무기 명명 — "${cnh}의 무기는 [구체 단어 1개]예요." (자녀 일주 60갑자 + 신살 결합으로 결정. 예: "단단한 결" "흐르는 결" "빛나는 결" 등) → ② 일주 메커니즘 + 일간 오행·신살(양인·괴강 등)·신강신약 결합 풀이 (180~220자) → ③ **빛나는 순간 묘사** ("이런 순간에 ${cnh}만의 결이 빛나요 — [구체 장면 1개]". 80~120자, 영화의 한 장면처럼) → ④ 부모가 알아볼 단서 한 줄. 무기는 자녀 사주 데이터에서만 도출. 일반론 X.

### 환하게 빛나게 해주는 결 한 가지
[메인: 용신 / 서브: 일간 오행·신강신약·12운성·계절]
[시그너처: 처방 — 일상 채움 가이드] — 부모가 일상에서 ${cnh}의 용신을 채워주는 구체 가이드를 색·활동·공간·계절·시간 5축으로 처방. 분량 500~600자.
구성: ① 도입 — "${cnh}이 환하게 빛나려면 채워줄 결 하나가 있어요. 자도인이 보는 그 결은 [용신 오행]이에요." 용신(用神) 한자 1회 → ② 용신 메커니즘 + 일간 오행과의 관계 + 신강·신약별 용신 강도 + 12운성·계절 결합 풀이 (180~220자) → ③ **5축 처방 박스**: 색 / 활동 / 공간 / 계절 / 시간. 각 한 줄씩 자녀 용신 오행 기반 구체 권장 (총 150~200자) → ④ 한 줄 정리. 용신=일간 동일 케이스("자기 결로 자라는 사주")도 시그너처 유지.

### 10대·20대·30대 어느 때 가장 빛날까
[메인: 대운 / 서브: 일간 오행·용신·세운·12운성]
[시그너처: 연령대 타임라인 — 그래프 묘사] — 10대~80대 대운을 시간선으로 펼치고 빛나는 연령대 1~2구간을 강조. 분량 500~600자.
구성: ① 도입 — "${cnh}의 빛은 평생 같은 세기로 흐르지 않아요" → ② 대운 메커니즘 — 자녀 대운 8개 흐름 + 용신 오행 진입 시기 + 12운성 강세(장생·관대·건록·제왕) 시기 결합 풀이 (180~220자) → ③ **타임라인 박스**: 10대 → 20대 → 30대 → 40대 ... 각 연령대 짧은 한 줄 묘사 (자녀 대운 데이터 기반, 강한 시기는 ★ 표시. 총 200~250자) → ④ 가장 빛나는 1~2구간 강조 한 줄. 단정 금지 — "큰 흐름은 ~한 시기에 강해 보입니다" 톤.

### 리더로 클까, 깊이 있는 전문가로 클까
[메인: 관성·인성 / 서브: 비겁·일주·신강신약]
[시그너처: 양 갈래 비교 + 자녀 위치 표시] — 리더형 vs 전문가형 두 갈래 정의 + 자녀 사주가 어디에 가까운지 위치 표시. 분량 500~600자.
구성: ① 도입 — "리더로 클지, 깊이 있는 전문가로 클지 — 두 결 모두 가치 있어요" → ② **두 갈래 정의**: 🌟 리더형 (관성 강·비겁 강 — 외부 책임·조직 운영, 한 단락 100~140자) / 📚 전문가형 (인성 강 — 깊이·연구·사색, 한 단락 100~140자) → ③ ${cnh}의 사주가 어디에 가까운지 위치 — 관성·인성 강도 비교 + 비겁·일주·신강신약 결합 풀이 (180~220자) → ④ 둘 다 성장 가능 한 줄 + 부모 권고. 관인상생 케이스는 "두 결 다 강한 사주" 결합 풀이.`;
