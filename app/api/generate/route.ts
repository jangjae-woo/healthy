...
1. **단락 1 — 일지의 의미** (1~2 문장): "**일지(日支)**는 *일상의 결이 자리하는 자리*"임을 풀이. 일지 관계 종류(육합·육충·형·해·파·비화·기타) 한자병기 1회.
2. **단락 2 — 두 분의 일지 관계 풀이** (2~3 문장): 위 시드의 풀이를 기반으로, 아버지와 자녀의 *일상 결*이 어떻게 만나는지 구체 장면 1~2개.
3. **단락 3 — 관계의 자원** (1~2 문장): 어떤 관계여도 의미가 있고, 흔들림이 있다면 자녀를 자라게 하는 자극이라는 톤.
...
본문 구조 (280~340자, 3 단락):
1. **단락 1 — 부모 십성의 의미** (1~2 문장): "자녀의 일간을 기준으로, ${d.dadName}님(아버지)의 일간은 자녀에게 **${dadParentSipseong?.sipseong ?? "—"}(${dadParentSipseong?.hanja ?? ""})**의 결입니다."로 시작.
2. **단락 2 — 아버지가 자녀에게 어떤 결로 작용하나** (2~3 문장): 위 시드의 풀이를 자연스럽게 풀어, 아버지가 자녀의 일상에서 어떤 결로 다가가는지 구체 장면 1~2개.
3. **단락 3 — 아버지 결의 자원과 살핌** (1~2 문장): 그 결의 자원과 과하면 살펴줄 점을 한 줄로 균형.
...
- ★ 시기 언급은 반드시 현재 이후만 (이미 지나간 과거 시기를 들춰내지 말 것). 다가올 시기는 대략적으로 ("앞으로 ~한 시기에", "자라가며" 등). ${childCrisis ? `🔴 **자도인 마지막 한마디에서 "사춘기·결이 변하는 시기" 언급 금지** — 그건 별도 ### 사춘기 페이지에서 이미 다루므로 자도인 마지막 한마디는 사주 본질·평생 메시지로 마무리 (Phase 4 어휘 분리).` : `("사춘기에 들어서면" 등)`}
