...
6. **마지막 한 줄 (한 문장)**: "이제 자도인이 두 분의 결을 차근차근 풀어드리겠습니다."`
...
- ✗ 십성 어휘 절대 노출 금지: 사색·받아들임·절제·규율·비겁·식상·재성·관성·인성·쥐는·다스리는 (이건 다음 페이지 "다섯 색깔의 결"에서 풀이)
...
위 사전 계산 결과를 바탕으로 한 단락(3~4문장)으로 풀어주세요. 합이 있으면 잘 어울리는 자극·시간·환경, 충이 있으면 부딪히는 자극·짜증의 자리. "충/합" 한자 명칭 본문 노출 금지.
...
- ✅ 오행별 구체 예시 권장:
  - 목(木): 자라나는 식물·공원·아침 햇살
  - 화(火): 환한 빛·따뜻한 사람·활기찬 활동
  - 토(土): 안정된 환경·일관된 일상·흙과 가까움
  - 금(金): 정돈된 공간·맑은 공기·결단의 활동
  - 수(水): 깊은 휴식·물과 가까움·사색의 시간
...
🔴 **영아·유아(0~72개월) 케이스 — 본문 반드시 미래 예고 톤**:
- 이 시기 자녀는 친구 갈등이 본격화되기 전 단계 (영아: 병행 놀이 / 유아: 단순 다툼)
- ❌ 절대 금지: "지금 ~한다", "갈등이 생기면 ~해주세요" 같은 현재형 단정
- ✅ 권장 어휘: "**앞으로** ${d.childName}${d.childGender === '남' ? '군' : '양'}이 친구를 사귀고 갈등이 생길 시기가 오면", "**자녀가 자라면서** 친구 관계가 본격화될 때", "**미리 알고 준비하실** 부모님 가이드" 같은 미래 예고·준비 톤
- ✅ 부모가 미리 알아두는 안내 형식. 현재 갈등이 있는 것처럼 단정 X.` : ''}
...
★★★ **반드시 이 형식 그대로 출력** (각 카드는 두 줄: 본문 + 📌 사주 근거):
