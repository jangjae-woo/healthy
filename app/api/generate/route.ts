...
- **신살 용어 첫 등장 슬롯 매핑** (이 슬롯에서만 정의 가능, 그 외 페이지는 참조만):
  ...
  - 복성귀인·천을귀인·태극귀인·천덕·월덕·금여·도화 → "### 인복의 결 — 복성·천을·도화" (Ch 4-3) 페이지에서만 정의 (D안 G)
...
| 천을·복성·태극·천덕·월덕·금여·도화 | "### 인복의 결 — 복성·천을·도화" (Ch 4-3 — D안 G PRIMARY) | 엄마/아빠 공통 신살 페이지(작용만) |
| 용신 | "### 자녀의 개운법(改運法)" (Ch 5-3 — D안 G PRIMARY 정의 + 비보 5채널) | 다른 페이지 한 줄 참조만 |
| 기신 | "### 절대 하면 안 되는 5가지" (Ch 5-5 — D안 G PRIMARY 정의 + 살핌 5가지) | 다른 페이지 한 줄 참조만 |
...
<!-- D안 G 시나리오 2: Ch 1의 기운 총량 페이지는 Ch 5 첫 페이지로 이전. 이 위치는 출력 금지. -->
★ **"### 기운 총량" 섹션은 어떤 연령에서도 출력하지 마세요** (Ch 1에서 — Ch 5로 이전됨, "### 기운 총량 — 자녀의 에너지 결이 일상에 어떻게 흐르나요"라는 제목이 Ch 5 첫 페이지에 있음).
...
2. **단락 2 — 자녀 단독 자리에서 어떻게 보이나** (2~3 문장, 약 130~170자): **자녀 단독 장면 1~2개**를 들어 자녀가 이 결을 어떻게 드러내는지. 🔴 **허용 장면만**: "혼자 일을 만났을 때", "새 환경에 들어섰을 때", "처음 보는 일에 닿았을 때", "자기 안에서 결정을 내릴 때", "어떤 일에 처음 마음을 정할 때". 🔴 **금지 장면**: "친구들과", "놀이를 할 때", "교실에서", "학교에서", "집에서 식탁/잠자리/취침" 등 — Ch 4·3·5 슬롯 침범 절대 X (D안 F 매트릭스 강제).
...
- **극약·태약·신약**: "에너지가 가벼운 편" / "혼자 결정·실행을 부담스러워할 수 있음" / "도움과 함께할 때 가장 자연스럽게 자기 결을 펼치는 자녀". 자녀 단독 장면 (친구·학교 어휘 X): "혼자 결정해야 하는 순간 평소보다 더 많이 망설인다", "새 환경에 들어섰을 때 한 발 뒤로 물러나 살피는 결".
- **중화**: "기운이 자연스럽게 어우러진 결" / "어느 한쪽으로 치우치지 않고 흐름을 잡는 자녀". 자녀 단독 장면: "처음 보는 일에 닿았을 때 자기 호흡으로 받아들이는 결", "자기 안에서 흔들림 없이 결을 잡는 자녀".
- **신강·태강·극왕**: "에너지가 단단히 자리 잡은 결" / "스스로 결정하고 끌어가는 추진력" / "고집을 부리거나 타협이 어려울 때가 있을 수 있는 결". 자녀 단독 장면 (친구·놀이 어휘 X): "한번 정한 일은 끝까지 밀고 나간다", "새 환경에서도 자기 안의 기준으로 결정을 내리는 결".
...
<!-- D안 G 시나리오 2: Ch 1의 격국 페이지는 Ch 7-1 (격국 진로 적성)에 흡수됨. 이 위치는 출력 금지. -->
★ **"### 격국(格局) — 자녀 인생의 큰 그림" 섹션은 어떤 연령에서도 출력하지 마세요** (Ch 1에서 — 격국 정의·풀이는 Ch 7-1 격국 진로 적성에서 PRIMARY).

### 격국(格局) — 자녀 인생의 큰 그림
※ 페이지 위에 자녀의 격국 카드가 자동 표시됩니다.

🔴 **D-3a 진짜 압축 룰 (양반사주 모델)**: 본 페이지는 격국 *한 줄 한눈* + *분산 안내*만. **격국 정의·뜻 풀이는 절대 X — Ch 7-1 (격국 진로 적성) 새 정의 슬롯에서만**.

★★★ **반드시 다음 결정론 시드만 사용** (Phase 2 신규 — 격국 풀이):
- ${childGyeokgukSeed}

본문 구조 (반드시 이 흐름, **80~120자, 1 단락만** — D-3a 진짜 한눈 압축):
1. **단락 1 — 격국 한 줄 + 분산 안내** (1~2 문장, 약 80~120자): "${d.childName}${d.childGender === '남' ? '군' : '양'}는 **[격국명]([한자])**을 타고난 자녀입니다 — [한 줄 의미]." 로 시작 → "이 결의 깊은 풀이는 *Ch 7 진로* 챕터에서 함께 살펴드리겠습니다." 톤으로 마무리. 단락 1개만, 깊은 풀이·일상 장면·인생 흐름 묘사 일체 X.

🔴 **어휘 정책**:
- ✅ 격국 한자병기 1회만 (예: "정관격(正官格)") — 첫 등장만.
- ❌ 점수·% 수치 본문 노출 금지.
- ❌ 격국 정의·뜻 풀이 일체 X (Ch 7-1로 분산).
- ❌ 일상 장면·인생 흐름·부모 가이드 일체 X — 한 줄 한눈만.
- ❌ 직업 단정 금지.
- ✅ 분산 안내 한 줄 ("Ch 7 진로 챕터에서") 반드시 포함.

🔴 **자원 프레임**: 자녀의 격국은 *타고난 본질*. 한눈만 — 인정·이해 톤.

<!-- D안 G 시나리오 2: Ch 1의 일주 캐릭터 페이지는 폐기 (일주 메타포는 차트 페이지 헤더에 이미 자동 표시됨, 풀이는 마음·학습·일상 챕터의 톤 가이드로 흡수). 이 위치는 출력 금지. -->
★ **"### 일주 캐릭터 — 60갑자의 결" 섹션은 어떤 연령에서도 출력하지 마세요** (Ch 1에서 — 일주 메타포는 차트 헤더 + 다른 챕터 톤 참조로 분산).

### 일주 캐릭터 — 60갑자의 결
※ 페이지 헤더에 "🎴 일주 60갑자" 라벨이 자동 표시됩니다.

🔴 **D-3a 진짜 압축 룰 (양반사주 모델)**: 본 페이지는 일주 *60갑자 캐릭터 한 줄* + *분산 안내*만. 일주 작용은 마음·학습·일상 챕터에서 *작용·장면*으로 분산.
...
본문 구조 (반드시 이 흐름, **80~120자, 1 단락만** — D-3a 진짜 한눈 압축):
1. **단락 1 — 60갑자 캐릭터 한 줄 + 분산 안내** (1~2 문장, 약 80~120자): "${d.childName}는 **[일주 한자]([일주명])**의 자녀입니다 — [60갑자 캐릭터 한 줄: 자연·동물·풍경 비유, 예: '햇살 머금은 흙의 결']." 로 시작 → "이 결의 작용은 마음·학습·일상 챕터에서 펼쳐집니다." 톤으로 마무리. 단락 1개만, 일상 장면 묘사·정의 풀이 일체 X.

🔴 **어휘 정책**:
- ✅ 일주 한자 1회만 본문에 (예: "계묘(癸卯)") — 첫 등장만.
- ❌ 일상 결 장면·6요인·십성·신살·기운 총량 영역 침범 X.
- ❌ 정의 풀이 일체 X (작용은 마음·학습·일상 챕터로 분산).
- ❌ 부모 가이드 동반 X — 한 줄 캐릭터만.
- ✅ 분산 안내 한 줄 반드시 포함.

<!-- D안 G 시나리오 2: Ch 1의 타고난 귀인 페이지는 폐기. 학당·문창귀인은 Ch 3 학습의 결, 천을·복성·태극·천덕·월덕·금여·도화는 Ch 4-3 인복의 결로 분산. 이 위치는 출력 금지. -->
★ **"### 타고난 귀인 (吉星)" 섹션은 어떤 연령에서도 출력하지 마세요** (Ch 1에서 — 학당·문창은 Ch 3 학습의 결, 천을·복성·태극 등은 Ch 4-3 인복의 결에서 PRIMARY).

### 타고난 귀인 (吉星)
※ 페이지 헤더에 "✨ 신살 (귀인)" 라벨이 자동 표시됩니다.

🔴 **D-3a 진짜 분산 룰 (양반사주 모델)**: 본 페이지는 자녀의 귀인 *이름 카드 한눈* + *분산 안내*만. **신살 정의·뜻 풀이 일체 X — 도메인 챕터로 완전 분산**:
- 학당귀인·문창귀인 작용 → **Ch 3 (학습 챕터)** 에서
- 복성귀인·천을귀인·도화살 작용 → **Ch 4-3 (인복의 결)** 새 페이지에서
- 월덕·천덕·태극·금여 작용 → 관계·일상 챕터에서
...
${(sajuChild.sinsal ?? []).filter(n => SINSAL_INFO[n]?.category === '귀인').length === 0
  ? `1. **귀인 없음 케이스 한 줄**: "${d.childName}${d.childGender === '남' ? '군' : '양'}의 사주에는 도드라진 귀인이 없는 안정형 사주입니다 — 외부 도움보다 자기 결로 차분히 자라가는 자녀입니다." 로 마무리. 단락 1개만, 정의·뜻 풀이 일체 X.`
  : `1. **단락 1 — 귀인 이름 한눈 + 분산 안내** (1~2 문장, 약 80~120자): "${d.childName}의 빛나는 별은 **[신살1·신살2·신살3]**입니다." 로 시작 → "각 별의 깊은 작용은 학습·관계·일상 챕터에서 풀어드립니다." 톤으로 마무리. 단락 1개만, **신살 정의·뜻 풀이 절대 X** (이름·한자만 노출, "문창귀인은 학문의 별" 같은 풀이 X).`}

🔴 **★ 정의·뜻 풀이 절대 금지 (D-3a 강화)**:
- ❌ "문창귀인은 학문·문장의 별" 같은 한 줄 정의도 금지.
- ❌ "천을귀인 = 위기 순간 도움" 같은 작용 풀이 금지.
- ❌ 일상 장면·부모 가이드 일체 X.
- ✅ 신살 이름·한자만 카드처럼 노출 + 분산 안내 한 줄.

🔴 **신살 한자는 첫 등장 1회만** (예: "천을귀인(天乙貴人)") — 이후 한글만.

<!-- D안 G 시나리오 2: Ch 1의 용신·기신 페이지는 폐기. 용신은 Ch 5-3 개운법, 기신은 Ch 5-5 절대 하면 안 5가지로 흡수. 이 위치는 출력 금지. -->
★ **"### 평생 빛나는 결 — 채움과 살핌 (용신·기신)" 섹션은 어떤 연령에서도 출력하지 마세요** (Ch 1에서 — 용신 정의·풀이는 Ch 5-3 개운법, 기신 정의·풀이는 Ch 5-5 5가지에서 PRIMARY).

### 평생 빛나는 결 — 채움과 살핌 (용신·기신)
${yongsinCtx}

🔴 **D-3a 진짜 압축 룰 (양반사주 모델)**: 본 페이지는 용신·기신 *양면 한 줄 한눈* + *분산 안내*만. 정의 풀이는 도메인 챕터로 완전 분산:
- 용신 *정의 + 비보 5채널 깊은 풀이* → Ch 5-3 (개운법) 새 정의 슬롯
- 기신 *정의 + 일상 5가지 회피 깊은 풀이* → Ch 5-4 (절대 하면 안 5가지) 새 정의 슬롯
본 페이지에서는 용신·기신 *오행 명시 + 양면 균형 한 줄*까지만 — 정의·구체 행동 일체 X.
...
본문 구조 (반드시 이 흐름, **120~180자, 1~2 단락** — D-3a 진짜 한눈 압축):

**[단락 1 — 양면 한 줄]** (약 80~110자)
"${d.childName}의 평생 채움 결은 **${childGisinData?.yongsin ?? "용신"}** 의 결, 살핌 결은 **${childGisinData?.element ?? "기신"}** 의 결입니다 — [양면 균형 한 줄 메시지: 채움이 자녀를 자라게 하고, 살핌이 자녀를 흐리지 않게 한다는 톤]." 형식. **용신·기신 정의·뜻 풀이 절대 X**. 구체 행동·일상 장면 일체 X.

**[단락 2 — 분산 안내] (선택, 약 40~70자)**
"용신 비보는 *Ch 5 개운법*에서, 기신 살핌은 *Ch 5 절대 하면 안 5가지*에서 자세히 풀어드립니다." 톤의 분산 안내 한 줄.

🔴 **★ 정의·뜻 풀이 절대 금지 (D-3a 강화)**:
- ❌ "용신 = 평생 채움 결" 같은 정의 한 줄도 금지 (Ch 5-3에서).
- ❌ "기신 = 용신을 극(剋)하는 오행" 같은 정의 한 줄도 금지 (Ch 5-4에서).
- ❌ "햇볕 산책 30분·노래·춤 놀이" 같은 구체 행동 일체 X (Ch 5-3에서).
- ❌ "단호함·비판·냉정" 부모 행동 경고 어휘 X (Ch 5-4 PRIMARY 이동).
- ✅ 오행 이름 + 양면 균형 한 줄 + 분산 안내만.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 2 마음 챕터의 *식상 작용 — 표현 결* 페이지. 식상 PRIMARY 정의는 앞 "다섯 색깔의 결" 에서 끝났음. 본 페이지는 **표현 영역에서의 깊은 작용·일상 장면**만.

🔴 **사주 인자 — 본 페이지의 메인 인자**: 자녀의 식상(食傷) 강·약 결 + 일간 ${sajuChild.ilgan}. 식상 정의·뜻 풀이 절대 X (앞 페이지 PRIMARY). 표현 결의 *작용·장면·부모 가이드*만.

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) **식상(食傷)의 결**로 ${d.childName}${d.childGender === '남' ? '군' : '양'}은 마음을 [말·행동·예술 중 결에 맞는 채널]로 펼치는 자녀입니다." 형식. 식상 강 = 풍부·즉각 표현 / 식상 약 = 속에 머금어 더디게 표현. 한 줄만, 정의 X.
2. **단락 2 — 일상 장면 단락** (약 110~140자): 표현 결이 *드러나는 구체 장면 1~2개*. 식상 강: "기쁜 일이 있으면 곧장 손짓·목소리로 풀어냄·종이 위에 그림으로 펼침" / 식상 약: "마음에 무언가 차도 한참 후 뒤에야 한 마디씩 흘러나옴·말보다 표정·자세에서 먼저 드러남". 자녀 결에 맞는 장면.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 부모님이 그 표현 결을 어떻게 받아주실 때 자라는지 — 식상 강 자녀에게는 "자녀의 말·그림·노래를 *재단 없이 들어주시는 자리*", 식상 약 자녀에게는 "자녀가 한 마디 흘릴 때 *서두르지 않고 받아주시는 호흡*". 즉시 적용 행동 1가지.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀의 표현 결이 자기 호흡으로 깊이 가꾸어가는 톤.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 2 마음 챕터의 *일간 강약 + 비겁·관성 작용* 페이지. 비겁·관성 PRIMARY 정의는 앞 "다섯 색깔의 결" 에서 끝났음. 본 페이지는 **화·짜증·기복 영역에서의 깊은 작용·일상 트리거**만.

🔴 **사주 인자 — 본 페이지의 메인 인자**: 자녀의 일간 ${sajuChild.ilgan} 강약 + 비겁·관성 결의 작용. 정의 풀이 절대 X.

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 일간 결과 자기 결을 세우는 비겁·절제하는 관성의 분포로, 자녀는 [어떤 자극]에서 마음이 출렁이는 결을 가집니다." 형식. 일간 강 + 비겁 강 = 자기 결을 침범당했다고 느낄 때 / 일간 약 + 관성 강 = 페이스 강요·갑작스런 변화에 흔들림 / 관성 약 = 충동·욕구 막혔을 때 욱함.
2. **단락 2 — 일상 장면 단락** (약 110~140자): *기복 트리거 장면 2개*. "친구가 자녀의 물건을 허락 없이 만질 때 / 부모가 자녀의 호흡보다 빠르게 재촉할 때 / 한 번에 여러 일을 끊어 넣을 때 / 자녀가 몰두 중인 일이 끊길 때 / 잠·밥 시간이 어긋날 때" 중 자녀 결에 맞는 장면.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 화·짜증이 올라올 때 부모님께서 *어떻게 다가가실 때 결이 잦아드는지* — "한 박자 늦춰 자녀의 호흡을 살피시기·이름을 붙여 마음을 알아봐 주시기·잠시 자리를 비켜 자녀가 스스로 가라앉을 시간 두기" 중 자녀 결에 맞는 행동 1~2개. 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀가 자기 결을 다스려가는 빛으로 자라는 톤.
...
🔴 **D안 E 깊이 재작성 (양반사주·청월당)**: Ch 2 (마음) 챕터의 *자기 결·자존* 작용 페이지. **일주·12운성 정의 풀이 절대 X** (PRIMARY는 Ch 1-2 일주 / Ch 3-3 / Ch 5-1). 본 페이지는 *자존감·내면 안정 작용·장면*만, 깊이 강화.

🔴 **사주 인자 — 본 페이지의 메인 인자**: 자녀 일주 ${sajuChild.pillars.day.stem}${sajuChild.pillars.day.branch} 의 자기 인식 결 + 12운성 단계 작용 (정의 X). 일주 작용 = "자기 결 본질" / 12운성 = "현재 결 단계의 호흡".

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 일주 결로 자녀의 자존은 [자기 결의 톤]에서 자라납니다." 형식. 일간 오행별: 木 = "스스로 자라남을 인정받을 때" / 火 = "표현이 닿아 빛날 때" / 土 = "자기 자리가 안정될 때" / 金 = "자기 기준이 존중될 때" / 水 = "사색의 깊이가 알아봐질 때".
2. **단락 2 — 단단해지는 자리 장면** (약 100~130자): 자녀의 자존이 *자라는 일상 장면 1~2개*. "조용히 자기 페이스로 마칠 때·작은 성취를 부모가 짚어 알아봐 줄 때·자기 의견이 끊기지 않고 끝까지 들리는 자리에서·숨고를 자리가 보장될 때" 중 자녀 결에 맞는 자리 묘사.
3. **단락 3 — 흔들리는 자리 + 부모 가이드 단락** (약 80~110자): *흔들리는 신호 1개* (비교·재촉·평가) + 부모님이 어떻게 받쳐주실 때 결이 다시 단단해지는지. "오늘 한 일 중 자녀가 골라 말하게 하시기·결과 대신 호흡을 살피시기" 중 1가지 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀가 자기 결·자기 박자로 자존을 다듬어가는 톤.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 2 (마음) 챕터의 *닫힘 신호 살핌* 페이지. 일주·일지 충돌 패턴 작용. 정의 풀이 X.

🔴 **사주 인자 — 본 페이지의 메인 인자**: 자녀 일주 + 일지 ${sajuChild.pillars.day.branch} 의 자기 보호 결. 자녀가 *자기 결을 지키려 닫는 자연 반응*으로 풀이 (병리화·문제화 절대 X).

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 일주 결과 일지 자리로, 자녀가 [어떤 자리]에서 마음을 잠시 닫는 결을 가집니다 — 닫힘은 자녀가 자기 결을 지키는 자연 호흡입니다." 일간 오행별: 木 = "결이 꺾일 위협" / 火 = "표현이 막힐 때" / 土 = "갑작스런 변화" / 金 = "자기 기준이 무시될 때" / 水 = "깊이 사색할 시간 부족".
2. **단락 2 — 닫힘 신호 일상 장면** (약 100~140자): 자녀가 마음 닫을 때 보내는 *구체 신호 2~3개*. "입을 잠시 닫고 한 마디도 안 함·평소 보던 책·놀이를 손에서 놓음·방·구석으로 자리 옮김·고개를 살짝 떨궈 시선을 피함·짧게 '몰라'·'괜찮아' 만 반복" 중 자녀 결에 맞는 신호.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 닫힘 신호 보일 때 부모님 다가가는 길 — "그 자리에서 곧장 묻지 않고 옆자리에 같이 앉아 호흡을 맞추시기·시간 두고 다음 날 아침 부드럽게 결을 짚어 물으시기·자녀가 먼저 말 꺼낼 때까지 기다리시기" 중 1~2가지. 다그침 절대 금지 톤.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 닫힘은 자녀의 결을 지키는 호흡, 곧 다시 펼쳐가는 결로 마무리.
...
🔴 **D안 E 깊이 재작성 (양반사주·청월당)**: 학당·문창 정의 슬롯이지만 본문은 *학습 채널 깊이 풀이*. 정의는 한 줄로 압축, 본문은 자녀의 *배움 결*을 깊이 그림.

🔴 **사주 인자 — 메인 인자 (D-3 정의 슬롯)**: **학당귀인·문창귀인 = 본 페이지 정의 슬롯**. 한 줄 정의만, 본문은 작용·장면.
- 자녀가 학당귀인·문창귀인 보유 시: 한자병기 1회 + 한 줄 정의 ("학문의 별·글의 별") OK.
- 자녀가 미보유 시: 인성·관성 결로 학습 가이드 (인성·관성 정의는 Ch 2-2/Ch 2-3 PRIMARY).
- 식상 도메인 어휘: 본 페이지는 **"말·글로 정리"** 어휘만 (PRIMARY "감정·예술 표현"은 Ch 2-1).

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "${d.childName}${d.childGender === '남' ? '군' : '양'}은 [학당(學堂)/문창(文昌)/인성·관성 결] 의 빛으로 배움을 [어떻게] 받아들이는 자녀입니다." 신살 한자병기 1회 + 한 줄 의미. 정의 재서술 X.
2. **단락 2 — 배움이 자라는 자리 일상 장면** (약 110~140자): 자녀의 학습 결이 어디서 또렷해지는지 *구체 자리 2개*. "한 권의 책을 깊이 읽어 자기 호흡으로 정리하는 자리·1:1 대화로 묻고 답하며 결을 펼치는 자리·말과 글로 정리할 때 빛나는 자리·움직이며 만져 익히는 자리·체계 따라 차곡차곡 쌓는 자리" 중 자녀 결에 맞는 자리 묘사.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 부모님이 함께 공부하실 때 어떤 호흡으로 다가가실 때 결이 자라는지 — "잠자리 들기 전 5분 오늘 배운 한 가지 골라 말하게 하시기·자녀가 자기 박자로 정리할 시간 보장하시기·평가 대신 호흡 살피시기" 중 1~2가지 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀의 배움 결이 자기 호흡으로 깊이 자라는 톤.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 3 학습 챕터의 *인성(흡수)·식상(정리·표현) 작용* 페이지. 인성·식상 정의 풀이 절대 X (PRIMARY는 Ch 2). 본 페이지는 *학습 흡수·정리 작용·장면*만.

🔴 **사주 인자 — 메인 인자**: 자녀의 인성(印星) 결 = "지식 흡수해 정리" 어휘만 사용 (D-3 어휘 분리). 식상 결 = "말·글로 정리" 어휘만 사용 (Ch 2-1 PRIMARY 어휘 "감정·예술 표현" 절대 X).

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 인성·식상 결로, 자녀는 새 지식을 [흡수→정리] 의 [어떤 호흡]으로 받아들입니다." 인성 강 = "한 번 들어온 지식 깊이 곱씹어 자기 안에 정리" / 인성 약 = "곧장 행동·시도로 익힘, 곱씹는 시간 짧음" / 식상 강 = "들은 것을 말·글로 풀어 바깥으로 정리" / 식상 약 = "안에 머금어 한참 후 정리".
2. **단락 2 — 흡수·정리 일상 장면** (약 110~140자): "수업 끝나고 혼자 노트에 차곡차곡 정리하는 자녀·집에 와서 부모에게 오늘 배운 것을 줄줄 풀어내며 정리하는 자녀·바로 손으로 만지고 만들어 보며 익히는 자녀·잠자리에 누워 천천히 곱씹어 다음 날 또렷해지는 자녀" 중 자녀 결 장면.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 자녀의 흡수·정리 결을 *어떻게 받쳐주실지* — "정리 시간이 더딘 자녀에게 재촉 대신 침묵 5분 보장·말로 풀어내는 자녀에게 듣는 자리 만들기·곧장 시도하는 자녀에게 결과 평가 대신 과정 짚어주기" 중 1~2가지.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀의 흡수·정리 결이 자기 박자로 깊이 가꾸어가는 톤.
...
🔴 **D안 E 깊이 재작성 (양반사주·청월당)**: Ch 3 (배움) 챕터의 *12운성 정의 슬롯 + 학습 환경·시간 깊은 작용*. 정의 한 줄 압축, 본문은 *환경·시간 깊이 풀이*.

🔴 **사주 인자 — 메인 인자 (D-3 정의 슬롯)**: **12운성(十二運星) 정의 슬롯** + 학습 시간·환경 작용 PRIMARY.

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락 (12운성 정의 슬롯)** (약 80~100자): "**12운성(十二運星)** = 일주의 12단계 결의 흐름 — 자녀의 결이 지금 어느 호흡 단계에 있는지 가리키는 별. ${d.childName}의 일주는 [현재 단계]의 결로, 학습 호흡이 [어떤 톤]에서 풀립니다." 한자병기 1회 + 한 줄 정의.
2. **단락 2 — 학습 시간·환경 일상 장면** (약 110~140자): 자녀에게 학습이 자연스러운 *시간대·환경 2~3개* 구체. "이른 아침 머리가 가장 맑은 자리·저녁 식사 후 차분히 가라앉은 시간·움직이며 소리 내어 외우는 호흡·조용한 책상에서 한 호흡으로 깊이 들어가는 자리·짧게 끊어 휴식과 함께 가는 호흡" 중 자녀 결 매핑.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 부모님이 함께 짤 학습 시간표 톤 — "자녀의 가장 또렷한 시간 30분 학습 자리 만들기·자녀가 흐려지는 시간에는 가벼운 활동으로 결 살리기·주말 한 시간 자기 페이스 학습 자리 보장" 중 1~2가지.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀 학습 결이 자기 박자로 깊이 가꾸어가는 톤.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 3 학습 챕터의 *학습 정체 신호* 페이지. 자녀 일간 ${sajuChild.ilgan} + 약한 오행 작용. 기신 메인 풀이 절대 X (Ch 5-4 PRIMARY).

🔴 **사주 인자 — 메인 인자**: 자녀 일간 결을 거스르는 학습 방식 + 약한 오행 결의 학습 정체 작용. 정의 풀이 X.

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 일간 결과 약한 결로, 자녀는 [어떤 학습 자리]에서 결이 닫히기 쉽습니다." 일간 오행별: 木 일간 = "한 호흡으로 길게 자라는 결을 자르는 짧은 끊김" / 火 일간 = "표현·발산 막히는 침묵 강요" / 土 일간 = "자기 페이스 무시한 갑작스런 진도 변경" / 金 일간 = "정리되지 않은 산만한 자료" / 水 일간 = "사색할 시간 없이 빠른 답 강요".
2. **단락 2 — 학습 정체 일상 신호** (약 110~140자): 자녀 학습 결이 닫힐 때 보이는 *구체 신호 2~3개*. "책 펴 놓고 한참 멍해지는 자녀·평소 좋아하던 과목인데 손에서 책을 놓는 자녀·짧게 '몰라'·'그냥' 만 반복·머리 아프다며 자리를 떠남·연필을 자꾸 만지작거리며 글씨가 흐려짐" 중 자녀 결 신호.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 정체 신호 보일 때 부모님 대처 — "그 자리에서 더 밀어붙이지 않고 잠시 호흡 끊어 자녀에게 5분 자유 시간 주시기·다음 날 다른 자리·다른 시간에 한 번 더 펼쳐주시기·과목 바꿔 다른 결로 결 살리기" 중 1~2가지.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 학습 정체는 자녀의 결이 보내는 신호, 호흡 살펴주실 때 다시 자라는 톤.
...
🔴 **D안 E 깊이 재작성 (양반사주·청월당)**: 본 페이지는 자녀의 *결이 사람을 끌어당기는 방식*을 깊이 그림. 사주 한 줄 + 일상 장면 + 부모 가이드 3 단락 강제.

🔴 **사주 인자 — 메인 인자**: **비견·겁재의 관계 작용 (보조 슬롯)** + 식상의 사람 표현. 비겁·식상 정의 풀이 X (PRIMARY는 Ch 2). 식상 도메인 어휘 = **"사람과 결 펼치기"** 만 (Ch 2-1 "감정·예술 표현"·Ch 3-1 "말·글로 정리" 절대 X).

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}${d.childGender === '남' ? '군' : '양'}의 비겁·식상 결로 자녀는 *${friendS.dominant}* 결로 사람과 첫 인연을 맺습니다." 비겁 강 = 같은 결 친구 끌어당김 / 비겁 약 = 다른 결 친구에게서 배움 / 식상 강 = 사람 곁에서 결을 펼침 / 식상 약 = 인연을 천천히 깊이 맺음.
2. **단락 2 — 친구 자리 일상 장면** (약 110~140자): 자녀가 *친구를 만나는 구체 자리 2개*. "처음 만난 자리에서 곧장 어울려 함께 뛰노는 자녀·관찰하다 마음 통하는 한 명에게 다가가는 자녀·1:1 깊은 대화에서 결이 또렷한 자녀·여러 친구 한가운데서 자연스럽게 자리를 잡는 자녀·먼저 다가오는 친구를 받아주는 자녀" 중 자녀 결 장면.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 부모님이 자녀 친구 자리를 받쳐주는 길 — "놀이터·동호회·가까운 이웃 자리 자연스럽게 마련·자녀의 1:1 친구 집 초대·여러 결 섞이는 자리 보장·자녀가 친구 수에 부담 느낄 때 침묵으로 받쳐주기" 중 1~2가지 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 친구 *수*가 아니라 자녀의 결이 *자라는 환경*을 살펴주실 때 깊이 자라는 톤.
...
🔴 **D안 E 깊이 재작성 (양반사주·청월당)**: 본 페이지 = 일지 PRIMARY 정의 슬롯. 정의 한 줄 압축, 본문은 *갈등 다가옴 + 자녀 회복 패턴 + 부모 다리* 3 단락 깊이.

🔴 **사주 인자 — 메인 인자**: **일지(日支) 합·충·형·해·파 PRIMARY 정의 슬롯** + 일간 강약 작용. 정의 한 줄 + 본문 깊이.

🔴 **차트 결과 — 반드시 본문 반영**: 권장 거리 = **${friendDist.label}** (위치 ${friendDist.position}/100, ${friendDist.position < 45 ? "가까이" : friendDist.position < 70 ? "중간" : "멀리"} 영역).
- ❌ 차트와 반대 묘사 절대 금지.
- ✅ 본문은 차트 라벨 (${friendDist.label.split(" — ")[0]}) 톤으로 일관.

본문 구조 (반드시 이 흐름, **320~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락 (일지 정의 슬롯)** (약 80~100자): "**일지(日支)** = 일상 결이 자리하는 자리, 자녀와 친구의 결이 만나 [합·충·형·해·파]의 5가지 결로 풀이됩니다. ${d.childName}의 일지 결로 갈등은 [어떤 톤]으로 다가오고, 회복은 [어떤 호흡]으로 풀립니다." 한자병기 1회 + 한 줄 정의.
2. **단락 2 — 갈등 다가옴 + 자녀 회복 일상 장면** (약 110~140자): 자녀에게 갈등이 다가오는 자리 + 회복 패턴 *구체 장면 2개*. "친구의 한 마디에 그날은 입을 닫고 다음 날 아침 다시 자연스럽게 말을 거는 자녀·즉시 화내고 풀고 함께 다시 노는 자녀·집에 와 곰곰이 곱씹다 며칠 후 자기 호흡으로 다시 다가가는 자녀·중간에 부모가 다리 놓아줄 때 가장 자연스럽게 풀리는 자녀" 중 자녀 결.
3. **단락 3 — 부모 가이드 단락 (권장 거리)** (약 90~120자): 권장 거리(${friendDist.label.split(" — ")[0]}) 톤으로 시점별 부모 행동 — "갈등 직후 자녀의 호흡 살피며 곁에 머무르시기·다음 날 아침 부드럽게 마음 묻기·자녀가 먼저 말 꺼낼 때까지 자리만 비워주시기·필요 시 부모가 다리 놓아 친구 부모와 가볍게 연결" 중 차트 라벨에 맞는 1~2가지 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀가 자기 호흡으로 흔들림에서 일어나가는 톤.

🔴 **★ 일지 PRIMARY 정의 슬롯**: 단락 1 정의 한자병기 1회 OK. 이후 엄마/아빠 일지 페이지에서는 정의 X — 본 페이지 단일 슬롯.${childAgeStage === "infant" || childAgeStage === "preschool" ? `
...
🔴 **D-3a 신설 페이지 (양반사주 모델)**: Ch 4 (사람) 챕터의 *인복 신살 정의 슬롯*. 복성귀인·천을귀인·도화살의 한 줄 정의 + 인연 끌어당기는 결 작용 풀이.

★★★ **반드시 다음 사주 데이터만 사용** (D-3a 신규 — 인복 신살):
...
본문 구조 (반드시 이 흐름, **240~280자, 2~3 단락** — D-3a 신설 영역 가이드):
1. **단락 1 — 사주 한 줄 (인복 신살 정의 슬롯)** (약 60~80자): "${d.childName}의 인복 신살은 **[복성/천을/도화 중 가진 것]**입니다." 톤. 가진 신살에 따라 한 줄 정의 (한자병기 1회):
   - **복성귀인(福星貴人)** 가졌다면: "평생의 복·행운을 끌어당기는 별"
   - **천을귀인(天乙貴人)** 가졌다면: "어려움에 귀한 도움·결정적 인연의 별"
   - **도화살(桃花殺)** 가졌다면: "매력·인기의 별 (어린이는 사랑받는 결로 작용)"
   - 신살 미보유: "${d.childName}의 일간 ${sajuChild.ilgan} 본질로 사람을 끌어당기는 결입니다."
2. **단락 2 — 영역 가이드 메인 (인연 끌어당기는 결)** (약 130~150자): 이 신살(또는 결)이 자녀의 인간관계에서 어떻게 *인연을 끌어당기는지* 일상 장면 1~2개. 어떤 환경에서 좋은 인연을 만나는지 (예: 도서관·동호회·이웃·학교·친구 집 등) 부모가 자연스럽게 마련해 주실 자리.
3. **단락 3 — 마무리 한 줄** (약 60~80자): 자녀가 *자기 결로 사람과 만나가는* 톤. 부모가 환경만 살펴주실 때 인연이 자연스럽게 자란다는 마무리.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 4 사람 챕터의 *비겁·관성 작용 — 단체 자리* 페이지. 정의 풀이 X (PRIMARY는 Ch 2-2).

🔴 **사주 인자 — 메인 인자**: 비견·겁재의 단체 작용 + 관성의 규율 결. 비겁 강 = 무리에서 결정권 / 비겁 약 = 받아주는 자리 / 관성 강 = 규율 따르는 결 / 관성 약 = 자유로운 결.

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 비겁·관성 결로 자녀는 무리 안에서 [어떤 자리]를 자연스럽게 차지합니다." 비겁 강+관성 강 = 앞장서는 결 / 비겁 강+관성 약 = 자유롭게 끌어가는 결 / 비겁 약+관성 강 = 무리의 약속을 받쳐주는 결 / 비겁 약+관성 약 = 곁에서 호흡 맞춰가는 결.
2. **단락 2 — 단체 자리 일상 장면** (약 110~140자): 자녀의 단체 자리 *구체 장면 2개*. "놀이터에서 친구들에게 놀이 규칙 정해주는 자녀·여러 친구 중간에서 대화 다리 놓는 자녀·반 모임에서 한 자리 잡고 묵묵히 받쳐주는 자녀·작은 무리 두세 명 안에서 가장 또렷하게 결을 펼치는 자녀·여러 친구 사이를 자유롭게 오가는 자녀" 중 자녀 결.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 부모님이 자녀의 단체 자리를 받쳐주는 길 — "자녀가 결정권을 가질 때 책임 함께 짚어주기·받쳐주는 자녀에게 자기 의견도 펼칠 수 있는 자리 마련·자유로운 결의 자녀에게 작은 약속의 호흡 가르치기" 중 자녀 결에 맞는 1가지 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀가 자기 결로 무리 안에서 자기 자리를 다듬어가는 톤.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 4 사람 챕터의 *부모 십성 작용 — 가족 자리* 페이지. 부모 십성 정의는 Ch 8-2 PRIMARY — 본 페이지는 *가족 자리·형제 관계 작용*만.

🔴 **사주 인자 — 메인 인자**: 부모 십성의 가족 작용 (정의 X) + 자녀 일주의 가족 결.

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 일주 결과 부모님과 만나는 결로, 자녀는 가족 안에서 [어떤 자리]를 자연스럽게 차지합니다." 자녀 일간 강 = 가족 안에서 자기 의견 또렷한 자리 / 자녀 일간 약 = 부모 결을 받아 자라는 자리 / 외동 케이스: 부모와 깊이 닿는 자리 / 형제 있는 케이스: 형제 사이에서 자기 결 자리.
2. **단락 2 — 가족 안 자리 일상 장면** (약 110~140자): 가족 자리 *구체 장면 2개*. "식탁에서 자기 의견 분명히 펼치는 자녀·부모 손길에 안기듯 자라는 자녀·형제 사이에서 평화 다리 놓는 자녀·형제와 자기 결 다투며 자라는 자녀·가족 결정에 자기 목소리 내기 시작하는 자녀·할머니·할아버지 곁에서 더 부드러워지는 자녀" 중 자녀 결.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 부모님이 가족 안 자녀 자리 받쳐주는 길 — "가족 결정에 자녀 의견 묻는 자리 만들기·형제 비교 대신 각자의 결 짚어주기·자녀가 가족 안에서 받는 자리·주는 자리 균형 살피기" 중 1~2가지.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀가 가족 안에서 자기 결로 깊이 자라가는 톤.
...
본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 결에서 ${weakestElem}의 결이 가장 가벼워, 자녀의 몸은 [해당 오행 → 신체 부위]의 결로 흐릅니다." 한 줄로 약한 오행 + 신체 매핑 노출. 정의 X.
2. **단락 2 — 살펴주실 신호 일상 장면** (약 110~140자): 약한 오행에 따른 *살펴주실 신호 2~3개*. 木 약: "오래 앉아 있을 때 다리 자주 풀어 흔드는·새벽 잠이 얕은" / 火 약: "손발이 자주 차가운·기운이 가라앉기 쉬운" / 土 약: "식욕이 들쑥날쑥·배가 자주 더부룩한" / 金 약: "감기가 자주 길어지는·피부가 건조해지는" / 水 약: "물 마시는 양이 적은·집중이 짧아지는" 중 자녀 결.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 일상에서 약한 오행 채워주실 길 — 木: "잠자리 들기 전 가벼운 스트레칭" / 火: "따뜻한 햇살 받는 산책" / 土: "같은 시간 따뜻한 식사" / 金: "환기·실내 습도" / 水: "물 자주 마시는 자리·조용한 사색의 시간" 중 1~2가지 즉시 적용.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀의 몸 결이 자기 호흡으로 단단해지는 톤.
...
본문 구조 (D-3a — 용신 PRIMARY 정의 슬롯 + 비보 5채널 깊은 가이드, 280~340자, 3 단락):
1. **단락 1 — 용신 정의 슬롯 (D-3a 신규 PRIMARY)** (약 70~100자): "**용신(用神)** = 평생 채움 결, 자녀를 자라게 하는 5행의 별. ${d.childName}의 용신은 **${childGisinData?.yongsin ?? "—"}**입니다." 형식으로 *용신 정의 한 줄 진짜 추가* (한자병기 1회) → "**개운법(改運法)**은 그 용신에 맞춰 일상을 살림하는 다섯 채널입니다." 톤 이어서. 용신·개운법 한자병기 각 1회.
...
🔴 **★ 용신 PRIMARY 정의 슬롯 (D-3a 신규)**: 본 페이지가 **용신(用神)의 새 PRIMARY 정의 슬롯**. 단락 1에서 정의 한 줄 진짜 추가. Ch 1-4 (평생 빛나는 결)는 *오행 양면 한 줄 한눈*만 — 정의는 본 페이지에서.
...
본문 구조 (D-3a — 기신 PRIMARY 정의 슬롯 + 일상 5가지 회피 + 부모 행동 경고 PRIMARY, **3 단락**):
1. **단락 1 — 기신 정의 슬롯 (D-3a 신규 PRIMARY)** (약 70~100자): "**기신(忌神)** = 용신을 극하는 결, 살펴주면 좋은 별. ${d.childName}의 기신은 **${childGisinData?.element ?? "—"}**입니다." 형식으로 *기신 정의 한 줄 진짜 추가* (한자병기 1회). 이 정의는 본 페이지에만.
2. **단락 2 — 가장 조심할 한 가지 + 부모 행동 경고 PRIMARY** (약 80~120자): "${d.childName}${d.childGender === '남' ? '군' : '양'}에게 가장 조심해야 할 한 가지는 **${dangerSorted[0].name}** 입니다." 로 시작 → 부모가 무의식적으로 하기 쉬운 말·행동 한 줄 경고로 마무리. 본 페이지가 부모 행동 경고("**단호함·비판·냉정·강요**") PRIMARY 슬롯이므로 이 어휘 본 페이지에서만 사용 OK.
3. **단락 3 — 일상 결 마무리 한 줄** (선택, 약 50~70자): 살핌이 자녀를 흐리지 않게 한다는 톤.

★ 카드의 5가지를 다시 나열하지 말 것. 카드 정보와 중복되지 않게 압축적으로.
★ 사주 근거 외 추가 십성 용어(비겁·식상·인성 등) 본문 노출 X — 일상 행동·말투·장면 중심.
★ 절대 5개 항목을 불릿으로 나열 X.

🔴 **★ 기신 PRIMARY 정의 슬롯 (D-3a 신규)**: 본 페이지가 **기신(忌神)의 새 PRIMARY 정의 슬롯**. 단락 1에서 정의 한 줄 진짜 추가. Ch 1-4 (평생 빛나는 결)는 *오행 양면 한 줄*만 — 정의는 본 페이지에서.
🔴 **★ 부모 행동 경고 PRIMARY 슬롯 (D-3a 강화)**: "단호함·비판·냉정·강요" 어휘는 본 페이지 단락 2에서만 사용. 다른 페이지에서 이 어휘 X.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 6 시기 챕터의 *대운 PRIMARY 정의 슬롯*. 자녀가 지금 어느 대운에 있고 어떤 흐름을 타는지 깊이 풀이.

★★★ **반드시 다음 사주 데이터만 사용** (D안 E 신규 — 현재 대운):
...
본문 구조 (반드시 이 흐름, **320~420자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락 (대운 정의 슬롯)** (약 90~120자): "**대운(大運)** = 10년마다 자녀에게 흐르는 큰 결의 흐름. ${d.childName}의 현재 대운은 **[대운 한자]** 의 결로, 자녀의 일간 ${sajuChild.ilgan}(${childSeed.ilganElement}) 결에 [어떤 작용]을 더해 흐릅니다." 한자병기 1회 + 한 줄 정의.
2. **단락 2 — 현재 대운 일상 흐름** (약 120~150자): 현재 대운이 자녀의 일상에 어떻게 작용하는지 *구체 장면 1~2개*. "지금 시기는 ~한 결이 자녀에게 살아나는 흐름·자녀가 [학습·관계·자기 결] 영역에서 ~한 호흡을 타고 있는 시기·앞으로 [몇 년 후] 다음 대운으로 결이 한 번 변환됨" 톤. 자녀 연령 무관 부담 없는 톤.
3. **단락 3 — 부모 가이드 단락** (약 80~110자): 현재 대운 결을 받쳐주실 부모님 시각 — "이 시기 자녀의 ~한 결이 자라가고 있으니 [구체 환경·태도]로 곁에서 받쳐주시면 좋습니다" 톤. 즉시 적용 1가지.
4. **단락 4 — 마무리 한 줄** (약 40~60자): 자녀가 지금 결을 자기 호흡으로 깊이 가꾸어가는 톤.
...
- ${childGyeokgukSeed}
- 6각 진로 레이더 결과 (D-3b 통합): TOP 3 = **${jobTop3}**, 약한 결 = **${jobAvoid.name}**

본문 구조 (D-3b — 격국 PRIMARY 정의 + 6각 진로 적합 통합, 280~360자, 3 단락):
1. **단락 1 — 격국 정의 슬롯 (PRIMARY)** (약 90~120자): "**격국(格局)** = 사주의 가장 강한 결로 정해지는 인생 큰 그림 — 자녀의 평생 결의 방향. ${d.childName}의 격국은 **${childGyeokgukData?.name ?? '—'}(${childGyeokgukData?.hanja ?? ''})**입니다." 형식으로 *격국 정의 한 줄 진짜 추가* (한자병기 1회).
2. **단락 2 — 격국 + 6각 진로 적합 통합 풀이** (2~3 문장, 약 130~160자): 격국 시드의 *적합 분야* 중 2~3개를 6각 레이더 TOP 3 (**${jobTop3}**) 와 *교차*해 자녀에게 가장 자연스러운 분야 1~2개 강조. 약한 결 (**${jobAvoid.name}**) 은 *부담이 되는 영역*으로 한 줄.
3. **단락 3 — 부모의 시각 한 줄** (약 70~90자): 격국 기반 직업은 *방향성*이지 *단정*이 아님. 자녀가 자라가며 자기 결로 직업을 선택하도록, 부모가 *큰 방향만 안내*하는 시각으로 마무리.

🔴 **★ 격국 PRIMARY 정의 슬롯 (D-3a 신규)**: 본 페이지가 **격국(格局)의 새 PRIMARY 정의 슬롯**. 단락 1에서 정의 한 줄 진짜 추가. Ch 1-1 (격국 한 줄)은 *한 줄 한눈*만 — 정의·뜻 풀이는 본 페이지에서.
...
🔴 **D안 E 신설 페이지 (양반사주·청월당 깊이)**: Ch 7 진로 챕터의 *약한 오행·기신 작용 — 영역 보강* 페이지. 약한 오행 어휘 분리 강제: 본 페이지 = "**영역 보강**" 어휘만 (Ch 5-1 "몸 결의 흐름"·Ch 2-3 "환경 채움" 어휘 절대 X).

🔴 **사주 인자 — 메인 인자**: 자녀 약한 오행 (현재: ${weakestElem}) + 기신 (${childGisinData?.element ?? "—"}) 의 진로 영역 작용. 정의 풀이 X (PRIMARY는 Ch 5).

본문 구조 (반드시 이 흐름, **300~400자, 3~4 단락** — D안 E 깊이 강제):
1. **단락 1 — 사주 근거 단락** (약 80~100자): "(앞서 본) ${d.childName}의 결에서 ${weakestElem}의 결이 가벼워, 자녀는 진로에서 [어떤 영역]에 부담을 느낄 수 있습니다." 한 줄. 약한 오행 → 진로 영역 매핑: 木 약 = 길게 자라남이 필요한 영역 / 火 약 = 빠른 표현·발산이 필요한 영역 / 토 약 = 안정·반복이 필요한 영역 / 金 약 = 정밀·정리가 필요한 영역 / 水 약 = 깊이 사색·몰입이 필요한 영역.
2. **단락 2 — 부담 자리 일상 장면** (약 100~130자): 자녀가 *부담 느낄 수 있는 영역 장면 1~2개*. 짧은 장면 묘사. 단정 X — 가능성 어조 강제.
3. **단락 3 — 보완의 길 부모 가이드** (약 90~120자): 약한 결을 자라며 채워가는 길 — "어린 시기부터 [해당 오행 환경]을 일상에 자연스럽게 녹여 주시기·자녀가 자라며 자기 페이스로 그 영역에 다가갈 자리 보장·강요·조기 진로 결정 절대 피하시기" 중 1~2가지. **D-3 기신 회피 어휘 분리: 본 페이지 = "조기 진로 결정·강한 영역 강요" 어휘만 (Ch 3-4·Ch 5-4 어휘 X).**
4. **단락 4 — 마무리 한 줄** (약 40~60자): 약한 결도 자라며 채워가는 빛, 자녀가 자기 결로 깊이 가꾸어가는 톤.
