...
1. **단락 1 — 사주 근거 풀이 + TOP1 채널 한 줄 소개** (약 110~140자): "${d.childName}는 ~한 사주 결로 인해 (예: 인성·수가 강해 / 토 결이 약해 / 화·목이 활발해), 3채널 중 ${lifestyleTop.name}이 가장 두드러진 결입니다." 형식. 위 desc 의 사주 근거 키워드를 본문에 그대로 풀이. *카드 본문을 그대로 반복하지 말고 사주 근거 한 줄에 집중.*
2. **단락 2 — 일상 적용 (구체 장면 — 단락 1과 *다른 메시지*)** (약 110~140자): TOP1·BOTTOM 채널을 자녀 일상에서 어떻게 챙겨주면 좋은지 *부모가 즉시 적용 가능한 구체 장면 1~2개*. 카드 desc를 *반복하지 말고*, 다른 일상 장면을 추가.
3. **단락 3 — 부모의 한 줄 마무리** (약 60~90자): 자녀의 결을 *헤아리는* 부모 톤으로 마무리. "이런 결을 부모님께서 헤아려 주실 때 자녀의 ~" 형식.
...
본문 구조 (반드시 이 흐름, 240~300자, 2~3 단락):
1. **단락 1 — 자녀의 활기 시간** (1~2 문장, 약 80~110자): 자녀의 일간 오행에 맞는 *가장 활기 있는 시간대*와 *가벼운 시간대*. 시드의 표현 그대로 인용.
2. **단락 2 — 잠자기·학습·야외 활동 시간** (2~3 문장, 약 110~140자): 시드의 잠·학습·야외 가이드 3가지를 자녀 일상에 적용해 풀이.
3. **단락 3 — 자녀의 일상 호흡** (1~2 문장, 약 60~90자): 자녀가 자신의 결에 맞는 시간대를 살아갈 때 가장 자기답게 자라는 결로 마무리.
...
🔴 **본문 구조** (반드시 이 흐름):
1. **시기 소개 한 줄**: "${d.childName}${d.childGender === '남' ? '군' : '양'}에게는 ${childCrisis.ageRange}이 결이 자연스럽게 변하는 시기로 다가옵니다."
...
2. **단락 2 — 두 분의 일지 관계 풀이** (2~3 문장, 약 110~140자): 위 시드의 풀이 한 줄을 기반으로, 어머니와 자녀의 *일상 결*이 어떻게 만나는지 구체 장면 1~2개. (예: 육합 → "아침 인사·식사·취침 같은 일상 호흡이 자연스럽게 맞아요", 육충 → "아침 일어나는 시간·취향이 부딪힐 수 있어요")
...
본문 구조 (반드시 이 흐름, 280~340자, 3 단락):
1. **단락 1 — 부모 십성의 의미** (1~2 문장, 약 90~110자): "자녀의 일간을 기준으로, ${d.momName}님(어머니)의 일간은 자녀에게 **${momParentSipseong?.sipseong ?? "—"}(${momParentSipseong?.hanja ?? ""})**의 결입니다."로 시작. 십성 한자병기 1회.
...
1. **단락 1 — 공통 신살의 명리학적 의미** (약 90~110자): "가족이 *공유하는 신살(神煞)*은 두 사주에 *같은 별이 자리*하는 경우입니다." 한자병기 1회. 명리학에서 이는 가족이 *한 차원에서 깊이 닮은* 인연을 뜻함을 명시.

2. **단락 2 — 두 분의 신살 *각자 명시*** (약 110~140자): **자녀와 어머님 각자의 신살을 한자병기로 모두 명시**. 형식: "자녀(${d.childName}${d.childGender === '남' ? '군' : '양'})의 신살: **${(sajuChild.sinsal ?? []).join('·') || "—"}** — [의미 한 줄]. 어머님의 신살: **${(sajuMom?.sinsal ?? []).join('·') || "—"}** — [의미 한 줄]." 각자 어떤 별을 타고났는지 *분명히* 보여줌.
...
1. **단락 1 — 명리 근거 명시** (약 80~100자): "어머님의 사주에 **[엄마 강 오행]의 결**이 강해서 자녀의 약한 **[자녀 약 오행]의 결**을 자연스럽게 채워주십니다." 형식. *왜 어머님이 그 결을 흐려주시는지* 사주 근거를 명확히.
...
1. **단락 1 — 시너지의 명리 근거** (약 80~100자): "위 시너지는 어머님의 **[엄마 강 십성/오행]**과 자녀의 **[자녀 강 십성/오행]**이 *동시 활성화*되는 자리에서 발생합니다." 형식. 시너지가 *왜 일어나는지* 명리 근거 한 줄.

2. **단락 2 — 일상 구체 장면 1~2개** (약 100~130자): 위 카드 중 1~2개를 골라 *주말·평일 어떤 일상 장면*에서 시너지가 가장 잘 일어나는지 구체화. 예: "주말 오후 베이킹·DIY·미술 활동에서 두 분의 결이 가장 잘 통하는 순간 — 자녀가 환히 웃는 표정이 자주 보입니다."

3. **단락 3 — 활용 가이드 한 줄** (약 60~80자): "이 시너지는 *주 1회 의식적으로* 마련해 주실 때 자녀의 자존감이 자랍니다." 형식의 활용 가이드 한 줄.
...
1. **단락 1 — 갈등 반복의 명리 근거** (약 90~110자): "이 갈등 패턴은 어머님의 **[엄마 강 십성]**과 자녀의 **[자녀 강 십성]**이 *서로 다른 차원*에서 작용해 발생합니다." 형식. *왜 이 갈등이 반복되는지* 명리 근거 한 줄.

2. **단락 2 — 갈등 발화 장면 구체화** (약 120~150자): 위 카드 중 가장 빈번한 갈등이 *어떤 일상 장면*에서 어떻게 시작·확대되는지 구체화. 예: "자녀가 정해진 일정을 어기거나 즉흥적으로 행동할 때, 어머님께서 즉각 제지하시면 자녀가 더 강하게 반발 — 갈등이 일주일에 2~3회 반복되는 패턴이 보일 수 있습니다."
...
1. **단락 1 — 명리 근거 명시** (약 70~90자): "어머님의 *일간 [한자]* 이 자녀의 *일간 [한자]* 에게 [생/극/비화/합] 의 자리이므로, 이 결을 의식적으로 살펴주시면 좋습니다." 형식. *왜 이 행동을 조절해야 하는지* 명리 근거 한 줄.

2. **단락 2 — 과한 케이스 한 줄 — 어떤 부작용** (약 70~90자): "이 결이 과해지면 자녀가 [구체 부작용 — 자기 결을 펼칠 자리를 잃거나·자기 호흡을 잃거나·...] 가 있을 수 있습니다." 형식.
...
2. **단락 2 — 아빠 영역 일상 장면** (2~3 문장): 위 시드의 풀이를 기반으로, 아버지와 자녀의 *일상 결*이 어떻게 만나는지 **아빠 영역(야외·운동·결단)** 구체 장면 1~2개.
...
본문 구조 (280~340자, 3 단락):
1. **단락 1 — 사실 진입** (1~2 문장): 십성 정의 재서술 X — 바로 "아버님의 일간은 자녀에게 [십성]의 결로 작용한다"는 사실 진입. 한자 1회만.
2. **단락 2 — 아빠 영역 일상 장면** (2~3 문장): 위 시드의 풀이를 풀어, 아버지가 자녀의 일상에서 어떤 결로 다가가는지 **아빠 영역(야외·결단·만들기)** 구체 장면 1~2개.
3. **단락 3 — 아버지 결의 자원과 살핌** (1~2 문장): 그 결의 자원과 과하면 살펴줄 점을 한 줄로 균형. 엄마 페이지의 결론 표현과 다른 어휘로.
...
2. **단락 2 — 두 분의 신살 *각자 명시*** (약 110~140자): **자녀와 아버님 각자의 신살을 한자병기로 모두 명시**. 형식: "자녀(${d.childName}${d.childGender === '남' ? '군' : '양'})의 신살: **${(sajuChild.sinsal ?? []).join('·') || "—"}** — [의미 한 줄]. 아버님의 신살: **${(sajuDad?.sinsal ?? []).join('·') || "—"}** — [의미 한 줄]." 각자 어떤 별을 타고났는지 *분명히* 보여줌.
...
1. **단락 1 — 명리 근거 명시** (약 80~100자): "아버님의 사주에 **[아빠 강 오행]의 결**이 강해서 자녀의 약한 **[자녀 약 오행]의 결**을 자연스럽게 채워주십니다." 형식. *왜 아버님이 그 결을 흐려주시는지* 사주 근거를 명확히.

2. **단락 2 — 일상 구체 장면** (약 100~130자): 아버님께서 자녀에게 그 결을 *어떤 일상 장면*에서 채워주시는지 1~2개. 아빠 특유의 *기둥의 결*(안정·방향성) 관점으로 구체화.
...
1. **단락 1 — 시너지의 명리 근거** (약 80~100자): "위 시너지는 아버님의 **[아빠 강 십성/오행]**과 자녀의 **[자녀 강 십성/오행]**이 *동시 활성화*되는 자리에서 발생합니다." 형식.

2. **단락 2 — 일상 구체 장면 1~2개** (약 100~130자): 위 카드 중 1~2개를 골라 *주말·평일 어떤 일상 장면*에서 시너지가 가장 잘 일어나는지 구체화. 아빠 특유의 *기둥의 결*(안정·방향성) 관점으로.

3. **단락 3 — 활용 가이드 한 줄** (약 60~80자): "이 시너지는 *주 1회 의식적으로* 마련해 주실 때 자녀의 자존감이 자랍니다." 형식.
...
1. **단락 1 — 갈등 반복의 명리 근거** (약 90~110자): "이 갈등 패턴은 아버님의 **[아빠 강 십성]**과 자녀의 **[자녀 강 십성]**이 *서로 다른 차원*에서 작용해 발생합니다." 형식.

2. **단락 2 — 갈등 발화 장면 구체화** (약 120~150자): 위 카드 중 가장 빈번한 갈등이 *어떤 일상 장면*에서 어떻게 시작·확대되는지 구체화. 아빠 특유의 *기둥의 결* 관점.
...
1. **단락 1 — 명리 근거 명시** (약 70~90자): "아버님의 *일간 [한자]* 이 자녀의 *일간 [한자]* 에게 [생/극/비화/합] 의 자리이므로, 이 결을 의식적으로 살펴주시면 좋습니다." 형식.

2. **단락 2 — 과한 케이스 한 줄 — 어떤 부작용** (약 70~90자): "이 결이 과해지면 자녀가 [구체 부작용] 가 있을 수 있습니다." 형식.
