...
### 절대 하면 안 되는 학습 방식
※ 이 페이지는 자녀에게 *역효과인 학습 방식*을 다룹니다. 사주 근거: 기신(${childGisinData?.element ?? "—"}) + 자녀 일간(${sajuChild.ilgan}) 거스르는 방식.

본문 구조 (반드시 이 흐름, 240~300자, 2~3 단락):
1. **단락 1 — 기신을 자극하는 학습 방식 사주 근거** (약 90~110자): 자녀의 *기신 오행*이 학습에서 어떻게 자극되는지 짧게 풀이. 정의 재서술 X — "(앞서 본) 기신의 결이 학습 자리에서는 ~한 방식이 부담"만.
2. **단락 2 — 부모가 피해야 할 구체 학습 행동** (약 100~130자): 부모가 무의식적으로 하기 쉬운 *역효과 학습 행동 2~3가지* 구체. 예: "다른 자녀와 비교 / 결과만 칭찬 / 빠른 답 강요 / 한 번에 여러 과목 / 자녀 페이스 무시한 시간표". 사주 근거에서 도출.
3. **단락 3 — 대안 한 줄** (약 50~70자): 그 대신 자녀에게 *자연스럽게 통하는* 한 가지 방식 한 줄.

🔴 **어휘 정책**:
- ❌ "절대 ~하지 마세요·~하면 망친다" 강한 협박 톤 X.
- ✅ "~한 방식은 자녀의 결을 *눌러* 학습 채널이 닫힐 수 있습니다" 가능성 어조.
- ❌ 부모 죄책감 어휘 X.
...
본문 구조 (반드시 이 흐름, 280~340자, 3 단락):
1. **단락 1 — 개운법의 의미** (1~2 문장, 약 80~110자): "**개운법(改運法)**" 한자병기 1회로 시작. 개운법 = 자녀의 *용신(평생 채울 결)*에 맞춘 *오행 비보(裨補)*임을 풀이. 자녀의 용신 오행을 명시.
