...
- ✅ 권장 패턴 (강 결 묘사):
  ...
  - "${d.childName}${d.childGender === '남' ? '군' : '양'}은 ${childSeed.ilganMetaphor}와 [강한 오행]이 어우러진 **본연의 결로 빛나는 자녀**입니다" (본질 톤 마무리 — "자라남" 같은 성장·확장 어휘 X, 본질 인정 톤만)
🔴 본질 페이지 마무리는 **본질 인정 톤** — "더 단단하게·더 깊이 자라남" 같은 보완 어휘 X. 본질은 보완 대상 아님.
...
### 외향-내향 스펙트럼
${d.childName}이 **밖으로 펼쳐지는 결**(외향 — 활동·표현) 과 **자기 안으로 모이는 결**(내향 — 사색·집중) 중 어느 쪽으로 기우는지를 일상 모습으로 풀어주세요.

🔴 **음양 어휘 본문 노출 금지** (사용자 인지 친화 — 옵션 B):
- ❌ "음 기운", "양 기운", "음양", "陰", "陽" 등 명리학 음양 용어 본문 절대 X
- ❌ 한자 "(陰氣運)", "(陽氣運)" 절대 X (후처리도 자동 제거)
- ✅ **시적 비유로만 풀이**: "자기 안으로 모이는 결", "밖으로 펼쳐지는 결", "안으로 깊이 흐르는 결", "활기차게 펼치는 결" 등
- ✅ 기존 "내향/외향" 어휘는 OK (한국어 친화)
...
🔴 **외향/내향은 자녀의 타고난 본질 — 보완·강화 대상 X** (사용자 발견 결함 처방):
- ❌ 절대 금지 마무리 패턴: "**더 단단하게 자라남**" / "부모님께서 ~ 다가가실 때 이 결이 자라남" / "~을 채워주시면 빛남" — 본질을 보완하는 톤 X.
- ❌ 부모 가이드 동반 X — 본질 페이지는 자녀 본질 인정만, 부모 행동 권고 자리 아님.
- ✅ 권장 마무리: "**이는 자녀가 타고난 본질이며, 자기다움으로 깊이 빛나는 결입니다**" / "**${d.childName}${d.childGender === '남' ? '군' : '양'}의 본연의 결로 자기답게 빛나는 자녀입니다**" 같이 본질 인정 톤만.
...
### 엄마와 함께 채울 결 — 외부 보완 가이드
※ 페이지 위에 외부 보완 카드 (둘 다 약한 결의 즉효·매일 행동) 가 자동 표시됩니다.

[차트 결과 — 본문에 인용]
- 엄마-자녀 둘 다 약한 결: ${momFlow?.bothLack.map(b => b.kor).join("·") || "(없음)"}
- 자녀 가장 약한 결 (회복카드 페이지에서 별도 처방): ${weakestElem}의 결
- 새 페이지에 표시될 element: 위 둘 다 약한 결 중 자녀 가장 약한 결 제외 (중복 회피)

본문은 한 단락(2~3줄). 부모-자녀 공통 약한 결을 외부에서 보완하는 의미만 짧게 설명. 카드 그리드에 구체 행동 (즉효·매일) 이 있으니 본문은 카드의 의미 안내 위주.
★ ❌ "더 단단하게·완성·강화" 같은 본질 변경 톤 금지 — "외부에서 부드럽게 채워주는 결" 톤.
★ Fallback 케이스 (둘 다 약한 결 없음): "두 분 모두 약한 결이 도드라지지 않아, 외부 보완 없이도 자연스럽게 어우러지는 결입니다" 톤.
...
### 아빠와 함께 채울 결 — 외부 보완 가이드
※ 페이지 위에 외부 보완 카드 (둘 다 약한 결의 즉효·매일 행동) 가 자동 표시됩니다.

[차트 결과 — 본문에 인용]
- 아빠-자녀 둘 다 약한 결: ${dadFlow?.bothLack.map(b => b.kor).join("·") || "(없음)"}
- 자녀 가장 약한 결 (회복카드 페이지에서 별도 처방): ${weakestElem}의 결
- 새 페이지에 표시될 element: 위 둘 다 약한 결 중 자녀 가장 약한 결 제외 (중복 회피)

본문은 한 단락(2~3줄). 부모-자녀 공통 약한 결을 외부에서 보완하는 의미만 짧게 설명. 카드 그리드에 구체 행동 (즉효·매일) 이 있으니 본문은 카드의 의미 안내 위주.
★ ❌ "더 단단하게·완성·강화" 같은 본질 변경 톤 금지 — "외부에서 부드럽게 채워주는 결" 톤.
★ Fallback 케이스 (둘 다 약한 결 없음): "두 분 모두 약한 결이 도드라지지 않아, 외부 보완 없이도 자연스럽게 어우러지는 결입니다" 톤.${hasMom ? '\n★ 엄마 섹션과 element 겹치는 결은 카드가 자동 처리 — 본문에서 중복 언급 X.' : ''}
