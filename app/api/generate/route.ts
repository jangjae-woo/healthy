...
🔴 **D-3a 신설 페이지 (양반사주 모델)**: Ch 2 (마음) 챕터의 *자기 결·자존* 작용 페이지. **일주·12운성 정의 풀이 절대 X** (PRIMARY는 Ch 1-2 일주 캐릭터 / Ch 3-3 효과적 학습 환경·시간 / Ch 5-1 잠자리·식습관). 본 페이지는 *자존감·내면 안정* 작용·장면만.

본문 구조 (반드시 이 흐름, **240~280자, 2~3 단락** — D-3a 신설 영역 가이드):
1. **단락 1 — 사주 한 줄** (약 60~80자): "${d.childName}${d.childGender === '남' ? '군' : '양'}의 자존감은 *일주의 결*과 *12운성 단계*에서 빛납니다." 톤. **일주 정의·12운성 정의 일체 X** — 작용 한 줄만.
2. **단락 2 — 영역 가이드 메인** (약 130~150자): 자녀의 자기 결이 *언제 흔들리는지·언제 단단해지는지* 일상 장면 1~2개. 부모가 어떻게 자존을 받쳐주실 때 결이 자라는지 구체 가이드. (예: "조용히 자기 시간을 가질 때 단단해지는 결 / 비교·재촉을 들을 때 흔들리는 결 / 작은 성취를 부모가 알아봐 줄 때 자라는 결").
3. **단락 3 — 마무리 한 줄** (약 60~80자): 자녀가 *자기 결·자기 호흡*으로 자라는 톤. 자존이 부모의 응원 속에서 자기 박자로 자란다는 마무리.

🔴 **어휘 정책**:
- ✅ 핵심 키워드: "**자기 결·자기 호흡·자존**".
- ❌ 일주 정의 풀이 X (Ch 1-2 PRIMARY).
- ❌ 12운성 정의 풀이 X (Ch 3-3·Ch 5-1 PRIMARY).
- ❌ "외향·내향·MBTI" 어휘 X.
- ❌ 부모 죄책감 어휘 X — "엄마가 못 챙기면" 같은 표현 X.
- ✅ 가능성 어조 ("~할 때 단단해질 수 있습니다·~한 결이 자라갑니다").
...
🔴 **D-3a 신설 페이지 (양반사주 모델)**: Ch 3 (배움) 챕터의 *12운성 정의 슬롯*. 12운성 한 줄 정의 한자병기 1회 OK + 학습 시간·환경 작용 풀이.

본문 구조 (반드시 이 흐름, **240~280자, 2~3 단락** — D-3a 신설 영역 가이드):
1. **단락 1 — 사주 한 줄 (12운성 정의 슬롯)** (약 60~80자): "**12운성(十二運星)** = 일주의 12단계 결의 흐름 — 절(絶)·태(胎)·양(養)·생(生)·욕(浴)·대(帶)·관(冠)·왕(旺)·쇠(衰)·병(病)·사(死)·묘(墓)." → "${d.childName}의 일주는 [현재 단계]에 있습니다." 톤. 한자병기 1회만, 한 줄 정의로 압축.
2. **단락 2 — 영역 가이드 메인 (학습 시간·환경)** (약 130~150자): 자녀 일주의 12운성 단계가 *학습 시간·환경*에 어떻게 작동하는지 — 어떤 시간대(아침/오후/저녁)·환경(조용한 책상/움직이며 익히기/소리 내며/반복 호흡)에서 학습 흡수가 가장 자연스러운지 구체 가이드 1~2개. 부모가 즉시 적용 가능한 장면.
3. **단락 3 — 마무리 한 줄** (약 60~80자): 자녀의 학습 결이 *자기 호흡*으로 자라는 톤. 시간·환경을 자녀 결에 맞춰주실 때 학습이 자기 박자로 자란다는 마무리.

🔴 **어휘 정책**:
- ✅ 12운성 한자병기 1회만 (단락 1 정의 슬롯) — 이후 한글만.
- ✅ 핵심 키워드: "**학습 호흡·학습 시간 결**" (일상 시간 결은 Ch 5-2 영역 — 어휘 분리).
- ❌ 일주 정의 풀이 X (Ch 1-2 PRIMARY) — 일주 작용 한 줄만.
- ❌ 일상 시간 결 어휘 X ("자연 활기 시간"은 Ch 5-2 영역).
- ❌ 신살 정의 풀이 X (Ch 1-3 PRIMARY).
- ✅ 가능성 어조.
...
🔴 **D-3a 헤더 변경 + 일지 PRIMARY 정의 슬롯 (양반사주 모델)**: 헤더가 "친구 갈등 시 부모 개입 거리" → "갈등·화해의 결"로 변경. 본문은 **일지(日支) 합·충·형·해·파 PRIMARY 정의 슬롯**으로 신설. 부모 개입 거리 슬라이더 카드는 그대로 보존.

🔴 **사주 인자 분배 — 이 페이지의 메인 인자**: **일지(日支) PRIMARY 정의 슬롯**. 한 줄 정의 한자병기 1회 OK + 갈등 회복 결 작용 풀이.
...
본문 구조 (D-3a — 일지 PRIMARY 정의 슬롯 + 갈등 회복 영역 가이드 메인, **240~280자, 2~3 단락**):
1. **단락 1 — 사주 한 줄 (일지 정의 슬롯)** (약 60~80자): "**일지(日支)** = 일상 결이 자리하는 자리 — 자녀와 친구의 일지가 만날 때 [합·충·형·해·파] 5가지 결로 풀이됩니다." 한자병기 1회 + 한 줄 정의. (PRIMARY 정의 슬롯이므로 일지 한자·정의 노출 1회 OK.)
2. **단락 2 — 영역 가이드 메인 (갈등·화해)** (약 130~150자): 자녀의 갈등 회복 결 — *어떻게 받아들이고 풀어내는지* 일상 장면 1~2개. 부모가 *어떤 태도·말투·시점*으로 다가가실 때 회복이 자연스러운지 권장 거리(${friendDist.label.split(" — ")[0]}) 톤으로 가이드. 갈등 직후 / 다음 날 / 자녀가 먼저 말 꺼낼 때 등 시점별 부모 행동 1~2개 구체화.
3. **단락 3 — 마무리 한 줄** (약 60~80자): 자녀가 *자기 호흡*으로 회복하는 톤으로 마무리.

🔴 **★ 일지 PRIMARY 정의 슬롯 (D-3a 신규)**: 본 페이지가 **일지(日支) 합·충·형·해·파의 새 PRIMARY 정의 슬롯**. 단락 1 정의 한자병기 1회 OK. 이후 엄마 일지 페이지(Ch 8) / 아빠 일지 페이지에서는 정의 금지 — 본 페이지가 새 단일 정의 슬롯.${childAgeStage === "infant" || childAgeStage === "preschool" ? `
