...
export function inferDangerCards(saju: SajuAnalysis): DangerCard[] {
  ...
  // ── sajuBasis 동적 분기 헬퍼 — Ch 8 양육 톤 (자원 프레임)
  // 1문장: 사주 사실 / 2문장: 두 분이 어떻게 다가가실 때 자녀 결이 자라는지 (자원 끝맺음)
  const basis = {
    // 1. 비교 — 비겁 카운트별 분기 (자존 강도)
    compare: bigyeop >= 2
      ? `사주의 비겁(자기를 세우는 결)이 ${bigyeop}로 강해 자기 자존이 단단한 자녀입니다. 두 분이 다른 자녀가 아닌 자녀만의 결을 비춰주실 때, 자녀의 자기 자존이 더 단단히 자라납니다.`
      : bigyeop === 1
        ? `사주의 비겁이 ${bigyeop}로 자기 결을 세워가는 시기의 자녀입니다. 두 분이 형성 중인 자기 인식을 비교 대신 인정으로 받쳐주실 때, 자녀가 자기 결을 활짝 펼쳐갑니다.`
        : `사주의 비겁이 0으로 자기 결을 다른 사람의 시선으로 가늠하기 쉬운 자녀입니다. 두 분이 자녀만의 결을 한 번 더 비춰주실 때, 자녀가 자기 자리를 단단히 찾아갑니다.`,
    // 2. 부정·비난 — 식상 카운트별 분기 (표현 욕구)
    deny: siksang >= 2
      ? `사주의 식상(표현하는 결)이 ${siksang}로 강해 마음을 바깥으로 풀어내려는 결이 큰 자녀입니다. 두 분이 결과 대신 자녀의 호흡을 함께 보아주실 때, 자녀의 표현 결이 자기 결로 활짝 펼쳐집니다.`
      : siksang === 1
        ? `사주의 식상이 ${siksang}로 표현이 신중한 자녀입니다. 두 분이 어렵게 꺼낸 한 마디를 결과로 평가하지 않고 받아주실 때, 자녀가 자기 결을 더 펼쳐갑니다.`
        : `사주의 식상이 0으로 표현이 더디고 안으로 모으는 결의 자녀입니다. 두 분이 자녀의 침묵을 결과로 단정하지 않고 호흡을 살피실 때, 자녀의 결이 자기 결로 자라납니다.`,
    // 3. 강요·지시 — 비겁·관성 조합별 분기 (자기 결 vs 절제 회로)
    force: bigyeop >= 2
      ? `사주의 비겁이 ${bigyeop}로 자기 결을 가려는 의지가 강한 자녀입니다. 두 분이 결정 시점을 5분만 늦추시고 자녀 의견을 먼저 들으실 때, 자녀의 자기 결이 단단해집니다.`
      : gwansong === 0
        ? `사주의 관성(절제 회로)이 0으로 자유로운 결의 자녀입니다. 두 분이 자녀의 욕구·충동을 살펴주시며 결정 시점을 늦춰 다가가실 때, 자녀가 자기 호흡으로 자라납니다.`
        : `사주의 비겁 ${bigyeop}, 관성 ${gwansong}으로 자녀의 결이 자기만의 흐름을 따릅니다. 두 분이 자녀의 결을 한 박자 뒤에서 지켜봐주실 때, 자녀가 자기 결로 펼쳐갑니다.`,
    // 4. 감정 살핌 — 인성·재성 카운트별 분기 (감수 깊이)
    ignore: insong >= 2 || jaesong >= 2
      ? `사주의 인성(${insong})과 재성(${jaesong})이 합쳐져 사람과의 결을 깊이 흡수하는 자녀입니다. 어머님이 식탁에서 자녀의 표정 한 번, 아버님이 야외에서 자녀의 호흡 한 번 살피실 때, 자녀의 내면 결이 단단히 자라납니다.`
      : (insong + jaesong) >= 1
        ? `사주의 인성 ${insong}, 재성 ${jaesong}으로 사람의 결을 조용히 흡수하는 자녀입니다. 두 분의 작은 살핌 한 번이 자녀의 마음에 깊이 닿아, 자녀의 자존이 단단해집니다.`
        : `사주의 인성·재성이 적어 감정 표현이 절제된 자녀입니다. 두 분이 자녀의 작은 신호를 한 번씩 알아봐주실 때, 자녀가 자기 감정을 자기 결로 펼쳐갑니다.`,
    // 5. 페이스·간섭 — 인성 카운트별 분기 (사색 공간)
    worry: insong >= 2
      ? `사주의 인성(사색하는 결)이 ${insong}로 강해 마음을 깊이 곱씹는 자녀입니다. 두 분이 한 박자 뒤에서 자녀의 결을 지켜봐주실 때, 자녀의 사색 공간이 활짝 열리고 자기 결정 회로가 단단해집니다.`
      : insong === 1
        ? `사주의 인성이 ${insong}로 사색이 형성 중인 자녀입니다. 두 분이 결정의 자리에 한 발 물러서 다가가실 때, 자녀의 자기 결정 회로가 자라납니다.`
        : `사주의 인성이 0으로 깊이 흡수하기보다 직관으로 움직이는 자녀입니다. 두 분이 자녀의 직관을 존중하시고 호흡을 맞춰주실 때, 자녀의 자율적 결정 결이 활짝 펼쳐집니다.`,
  };

  const raw: Record<string, { v: number; why: string; sajuBasis: string }> = {
    "다른 자녀와 비교하는 말": {
      v: bigyeop * 1.5 + (elem.화 ?? 0) * 0.4,
      why: "자녀의 결은 자녀만의 흐름이라 두 분이 자녀 결을 비춰주실 때 자기 호흡으로 자랍니다",
      sajuBasis: basis.compare,
    },
    "결과만 평가하는 말": {
      v: siksang * 1.5 + (elem.화 ?? 0) * 0.4,
      why: "자녀의 결은 결과로 단정되지 않습니다 — 두 분이 호흡을 보아주실 때 자기 결로 자랍니다",
      sajuBasis: basis.deny,
    },
    "자녀 호흡 앞서간 결정": {
      // 옵션 3 점수 보정: 비겁 0 케이스에선 가중치 낮춤 (sajuBasis 와 일치)
      v: bigyeop >= 1
        ? bigyeop * 1.5 + (elem.목 ?? 0) * 0.5 + Math.max(0, 3 - gwansong)
        : (elem.목 ?? 0) * 0.4 + Math.max(0, 2 - gwansong),
      why: "자기 결로 가고 싶은 자녀라 두 분이 한 박자 늦춰 다가가실 때 자녀가 자기 호흡으로 자라납니다",
      sajuBasis: basis.force,
    },
    "마음 한 번 읽지 못한 다음 행동": {
      v: insong * 1.0 + jaesong * 1.0 + (elem.토 ?? 0) * 0.4,
      why: "인정받고 싶은 결이 강한 자녀라 두 분의 작은 살핌이 자녀의 자존을 받쳐줍니다",
      sajuBasis: basis.ignore,
    },
    "자녀 페이스 앞서간 손길": {
      v: insong * 1.5 + (elem.수 ?? 0) * 0.5,
      why: "자녀가 자기 결을 펼칠 자리에 두 분 손길이 먼저 닿으면 결이 흐려질 수 있습니다",
      sajuBasis: basis.worry,
    },
  };
  const max = Math.max(...Object.values(raw).map((r) => r.v), 1);
  return Object.entries(raw).map(([name, r]) => ({
