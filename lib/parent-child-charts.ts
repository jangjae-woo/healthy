...
export function inferDangerCards(saju: SajuAnalysis): DangerCard[] {
  ...
  // ── sajuBasis 동적 분기 헬퍼 (옵션 5: 1+3 결합) ──
  // 사주 카운트별 정확한 텍스트 생성. 점수 공식과 의미적으로 일치 보장.
  const basis = {
    // 1. 비교 — 비겁 카운트별 분기 (자존 강도)
    compare: bigyeop >= 2
      ? `사주의 비겁(자기를 세우는 결)이 ${bigyeop}로 강해 자기 자존이 단단한 자녀입니다. 다른 아이와의 비교는 그 단단한 자존을 직접 깎아내리는 칼이 되어, 자녀의 마음을 깊이 다치게 합니다.`
      : bigyeop === 1
        ? `사주의 비겁이 ${bigyeop}로 자기 결을 세워가는 시기의 자녀입니다. 비교하는 말은 형성 중인 자기 인식을 흔들어, 자기 결을 펼치기 전에 위축되게 만듭니다.`
        : `사주의 비겁이 0으로 자기 결을 다른 사람의 시선으로 가늠하기 쉬운 자녀입니다. 비교는 자녀가 자기 자리를 잃고 다른 아이의 결과만 좇게 만듭니다.`,
    // 2. 부정·비난 — 식상 카운트별 분기 (표현 욕구)
    deny: siksang >= 2
      ? `사주의 식상(표현하는 결)이 ${siksang}로 강해 마음을 바깥으로 풀어내려는 결이 큰 자녀입니다. 이 표현 욕구를 부정·비난으로 막으면 자녀는 점점 입을 닫고 안으로만 삭이게 됩니다.`
      : siksang === 1
        ? `사주의 식상이 ${siksang}로 표현이 신중한 자녀입니다. 어렵게 꺼낸 마음을 부정·비난으로 받으면 자녀는 다시는 입을 열지 않으려 할 수 있습니다.`
        : `사주의 식상이 0으로 표현이 더디고 안으로 모으는 결의 자녀입니다. 부정·비난은 자녀의 마음을 더 깊은 침묵으로 밀어 넣게 됩니다.`,
    // 3. 강요·지시 — 비겁·관성 조합별 분기 (자기 결 vs 절제 회로)
    force: bigyeop >= 2
      ? `사주의 비겁이 ${bigyeop}로 자기 결을 가려는 의지가 강한 자녀입니다. 일방적 강요는 이 자기 결과 정면 충돌해 오히려 더 단단한 고집으로 돌아옵니다.`
      : gwansong === 0
        ? `사주의 관성(절제 회로)이 0으로 자유로운 결의 자녀입니다. 일방적 강요는 절제 회로가 약한 자녀에게 충동적 반발을 일으킵니다.`
        : `사주의 비겁 ${bigyeop}, 관성 ${gwansong}으로 자녀의 결이 자기만의 흐름을 따릅니다. 일방적 강요는 자녀가 자기 결을 보호하려 마음 닫게 만듭니다.`,
    // 4. 감정 무시 — 인성·재성 카운트별 분기 (감수 깊이)
    ignore: insong >= 2 || jaesong >= 2
      ? `사주의 인성(${insong})과 재성(${jaesong})이 합쳐져 사람과의 결을 깊이 흡수하는 자녀입니다. 감정을 모른 척하거나 무시하면 "내 마음은 가치 없다"는 신호로 받아들여 자존감이 무너집니다.`
      : (insong + jaesong) >= 1
        ? `사주의 인성 ${insong}, 재성 ${jaesong}으로 사람의 결을 조용히 흡수하는 자녀입니다. 감정을 무시당하면 마음에 작은 자국이 깊이 새겨집니다.`
        : `사주의 인성·재성이 적어 감정 표현이 절제된 자녀입니다. 무시하는 태도는 자녀가 감정 자체를 숨기는 결로 굳어지게 만듭니다.`,
    // 5. 과도한 걱정·간섭 — 인성 카운트별 분기 (사색 공간)
    worry: insong >= 2
      ? `사주의 인성(사색하는 결)이 ${insong}로 강해 마음을 깊이 곱씹는 자녀입니다. 부모의 걱정·간섭까지 무겁게 떠안으면 자기 사색의 공간이 좁아져 숨이 막히게 됩니다.`
      : insong === 1
        ? `사주의 인성이 ${insong}로 사색이 형성 중인 자녀입니다. 과도한 걱정·간섭은 자녀가 스스로 결정하는 회로를 약화시킵니다.`
        : `사주의 인성이 0으로 깊이 흡수하기보다 직관으로 움직이는 자녀입니다. 과도한 간섭은 자녀의 자율적 결정 회로를 흐리게 만듭니다.`,
  };
