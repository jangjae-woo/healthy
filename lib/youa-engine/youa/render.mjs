...
const TRACE_LABELS = {
  식상: '표현하고 움직이게 하는 기운<br/>(식상·<span class="han">食傷</span>)',
  양인: '결단하고 치고 나가는 기운<br/>(양인·<span class="han">羊刃</span>)',
  신강: '자기 에너지가 단단한 결<br/>(신강)',
  화: '밝고 빠르게 드러나는 불의 결<br/>(화·<span class="han">火</span> 오행)',
  목: '자라나고 뻗어가는 나무의 결<br/>(목·<span class="han">木</span> 오행)',
  ...
  수: '사주 가운데에 자리한 깊은 물의 결<br/>(수·<span class="han">水</span> 오행 강세)',
  인성: '받쳐주는 기운이 강해 표현의 결을 누름<br/>(인성·<span class="han">印星</span> 강)',
  '12운성 약세': '에너지가 잠긴 자리<br/>(12운성 약세)',
  관성: '절제하는 기운<br/>(관성·<span class="han">官星</span>)',
  토: '단단히 받쳐주는 흙의 결<br/>(토·<span class="han">土</span> 오행)',
  화개살: '안으로 깊어지는 기운<br/>(화개·<span class="han">華蓋</span>)',
  '관성 무존재': '절제 기운이 약한 자리<br/>(관성 없음)',
  '식상 강': '표현 기운이 앞서는 자리<br/>(식상 강)',
  조후: '따뜻함과 차가움의 균형<br/>(조후·<span class="han">調候</span>)',
  천을귀인: '부드럽게 보호하는 상서로운 기운<br/>(천을귀인·<span class="han">天乙貴人</span>)',
  관인상생: '절제와 받침이 이어지는 흐름<br/>(관인상생)',
  문창귀인: '섬세하게 알아차리는 기운<br/>(문창귀인·<span class="han">文昌</span>)',
  일주: '마음 자리의 안정 흐름<br/>(일주 안정)',
  충: '흔들리는 결이 마음 자리에 자리<br/>(충·<span class="han">沖</span>)',
  해: '속으로 스미는 불편감<br/>(해·<span class="han">害</span>)',
  원진: '오래 남는 감정의 결<br/>(원진·<span class="han">怨嗔</span>)',
  형: '긴장을 만드는 관계 자리<br/>(형·<span class="han">刑</span>)',
  합: '자연스럽게 통하는 결<br/>(합·<span class="han">合</span>)',
...
export function renderPage3Overview(facts) {
  return `
    <div class="page">
      ...
      <div class="chapter-header ch-outro">📊 ${facts.child.fullTitle}의 결 한눈에</div>
      <div class="score-box">
...
function renderFactorChapter(facts, factorKey, pageNum, chapterNum) {
  ...
  const c = FACTOR_COLORS[factorKey];
  const factorKo = f.factorKorean;
  const shortText = FACTOR_SHORT[factorKey][f.level === 'low' ? 'low' : 'high'];
  ...
  return `
    <div class="page">
      ...
      <div class="score-box">
        ${facts.child.fullTitle}의 ${factorKo}${josa(factorKo, '은', '는')} ${LEVEL_KO[f.level]}으로 드러나는 결입니다.<br/>
        <strong>${shortText}</strong>입니다.
      </div>
      <h3>왜 이런 결인가</h3>
      ${llm(facts, `${chKey}/왜 이런 결인가`, `${factorKo} 본문 산문 250~300자`)}
      ${renderFactorEvidenceTable(facts, factorKey)}
      ${renderFactorMechanism(facts, factorKey)}
      ${renderFactorRichInterpretation(facts, factorKey)}
      ${renderDailyScene(facts, factorKey)}
      <div class="tip-box">
        <div class="tip-label">👨‍👩‍👧 양육 Tip — ${factorKo}${josa(factorKo, '이', '가')} ${LEVEL_KO[f.level]}인 결의 자녀에게</div>
        ${renderTipItem(tipParts, 0, '시간 리듬을 맞춰 주십시오', '시간 축 80~120자')}
        ${renderTipItem(tipParts, 1, '소통 방식을 맞춰 주십시오', '소통 축 80~120자')}
        ${renderTipItem(tipParts, 2, '환경을 맞춰 주십시오', '환경 축 80~120자')}
      </div>
    </div>
  `;
}
...
function renderParentSajuPage(facts, role, pageNum) {
  ...
  return `
    <div class="page">
      ...
      <div style="margin-top:14px;">${llm(facts, `${role === 'mother' ? '9장' : '10장'} — ${label} 사주의 결`, `${label} 일간 비유 + 사주 큰 흐름 200~250자`)}</div>
      <h3>${label} 사주에 자리한 6가지 기운</h3>
      <p style="font-size:11px;color:#888;margin-bottom:8px;">탭하면 작용 디테일이 펼쳐집니다</p>
      ${renderFactorCards(cards)}
      <p style="margin-top:24px;">${label} 사주의 큰 흐름은 ${topFactors}이 본기로 자리해 단단한 결을 이룹니다. 이 결이 ${facts.child.fullTitle}의 결과 만나는 자리는 본 장 마지막 "함께 살펴줄 결"에서 정리됩니다.</p>
    </div>
  `;
}
...
export function renderPage17Outro(facts) {
  ...
  if (llmText) {
    ...
    body = `
      ...
      ${llmPlaceholder('아이 기질 이해의 가치 3문장')}
      ${llmPlaceholder('부모 자기 결 이해의 가치 2문장')}
      ${llmPlaceholder('두 결의 만남 2문장')}
