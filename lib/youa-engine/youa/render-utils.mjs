...
export function renderSaju8Box(title, pillars, info) {
  ...
  for (const k of order) {
    const isDay = k === 'day';
    h += `<div class="saju8-cell${isDay ? ' day' : ''}"><div class="pos">${labels[k]}</div><div class="char">${stemHan[pillars[k].stem]}</div></div>`;
  }
  for (const k of order) {
    const isDay = k === 'day';
    h += `<div class="saju8-cell${isDay ? ' day' : ''}"><div class="pos"></div><div class="char">${branchHan[pillars[k].branch]}</div></div>`;
  }
  h += `</div>`;
