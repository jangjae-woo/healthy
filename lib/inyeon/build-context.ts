...
▸ 적용 룰: 본문 어조·연애 묘사·갈등 묘사·미래 묘사 모두 위 두 캐릭터 결을 의식하고 전개.
   - ${aMatch.name} 캐릭터의 결로 ${aName}님 행동·심리 묘사
   - ${bMatch.name} 캐릭터의 결로 ${bName}님 행동·심리 묘사
...
2. **한 단락에 1개 이상 "캡처할 한 줄"**: 비유·시적 표현·구체 일상 장면이 들어간 짧은 명문 한 줄을 단락 안에 자연 배치.
3. **그 사람 행동을 사주 결로 통역**: 무관심·이기심·바람기 같은 부정 해석 절대 X. 사주 인자 결로 자연스럽게 풀이.
4. **자기 매력 환원**: 학술 톤("식상이 강하시네요") 금지. 그 인자가 만드는 매력·행동으로 풀어쓸 것.
