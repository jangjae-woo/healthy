...
### 나의 타고난 성격
${a.name}님이 어떤 결을 타고났는지. 일간 ${a.ilgan}(${a.ilganNature}) 자연 비유로 시작 + 일지에 깔린 기운 + 신살 1~2개 결합. 겉으로 비치는 모습과 속의 결이 어떻게 다른지 한 단락 포함. ${a.shinkang}이라는 신강신약의 결도 자연스럽게 녹임.

### 연애할 때 드러나는 나의 매력
${a.name}님이 연애 자리에 들어갔을 때 자연스럽게 드러나는 매력. 식상·재성 분포 + 도화살·홍염살·천을귀인 등 매력 신살 보유 시 그것을 매력 결로 연결. 그 사람이 ${a.name}님에게 끌리는 구체 매력 포인트 한두 가지를 일상 장면으로 (예: 카페에서 마주 앉았을 때, 함께 걷는 길에서, 메시지의 결).

### 내가 끌리는 이상형
${a.name}님이 어떤 사람에게 깊이 끌리는지. 부족한 오행 + 정관/편관/정재/편재 분포 + 일간 음양으로 본 끌리는 결. 외모보다 내면 결 중심 — "이런 결을 가진 사람을 보면 마음이 흔들려요" 톤. 추상 키워드 X — 구체 행동·태도로.

## ${b.name}님의 사주 — 이번 풀이의 "그 사람"

### 그 사람의 타고난 성격
${b.name}님 일간 ${b.ilgan}(${b.ilganNature}) 자연 비유 + 일지·신살 결합. ${b.shinkang}의 결을 풀이. ${a.name}님과 어떻게 다른 결인지 자연스럽게 드러나도록 (대비). 겉과 속의 차이 한 줄 포함.

### 그 사람이 연애할 때 보이는 모습
${b.name}님이 연애에 들어갔을 때 어떤 결로 다가가는지. 식상·재성·관성 결합 + 표현 방식·다가가는 거리·마음 표현 속도. ${a.name}님 입장에서 "그 사람의 이런 모습이 보일 거예요" 톤. 신살 매력 인자(있는 경우) 자연 결합.

### 그 사람이 끌리는 이상형
${b.name}님이 어떤 결의 사람에게 끌리는지. 부족한 오행 + 관성·재성·인성 분포 + 일간 음양 결합. ${a.name}님의 결이 그 이상형에 어떻게 가까운지(또는 다른지) 짧게 짚어 자연스러운 흐름 만들기.
