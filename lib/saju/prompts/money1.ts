...
[★★★ V2.4.0 인생 4단계 재산 흐름 강화 (money1-1 sub 의무 룰)]
sub 1 소제목은 **"### 인생 4단계 재산 흐름 (초년·청년·중년·말년)"** 으로 정확히 출력.
본문은 4단계 각각 별도 단락으로 1단락씩 풀이 (한 단락에 모두 묶지 말 것):
1. **초년기 (10~20대)** — dataBlock 〖4단계 재산 곡선〗의 초년기 점수와 일치하는 결로 1단락
2. **청년기 (30~40대)** — 청년기 점수와 일치하는 결로 1단락
3. **중년기 (50대)** — 중년기 점수와 일치하는 결로 1단락
4. **말년기 (60대+)** — 말년기 점수와 일치하는 결로 1단락. 차트가 낮으면 "관리 다지는 결" / 높으면 "도약하는 결"로 차트와 일치
※ 차트 점수 숫자 직접 인용 X — "재성이 강하게 들어오는 결" 같이 결 톤. 단정 액수 절대 X.
...
${injectLifetimePoolsBlock({
  ...
  scenarios: [
    ...
    // V2.4.0: m1_reality_choice 시나리오 풀 미사용 (money1-4 sub 삭제)
  ],
})}
