...
const SIX_FACTOR_WEAK_PHRASE: Record<string, Record<AgeStage, string>> = {
  활동성: {
    infant: "활발한 움직임은 부드러운 결로, 차분한 환경에서 자기 페이스로 자라나는 결",
    preschool: "왁자지껄한 곳보다 차분한 환경에서 더 편안한 결",
    elementary: "활동적인 자극보다 차분한 환경에서 자기 결을 키우는 편",
    secondary: "강한 활동성보다 차분한 자기 페이스에서 자기다움을 발휘하는 편",
  },
  표현력: {
    infant: "감정·생각을 신중히 마음에 머금는 결",
    preschool: "말로 펼치기보다 관찰·사색이 우선인 결",
    elementary: "표현보다 속으로 깊이 정리하는 결이 우선",
    secondary: "겉으로 펼치기보다 자기 안에서 다지는 결",
  },
  감수성: {
    infant: "강한 자극에도 흔들리지 않는 단단한 결",
    preschool: "감정 변화에 휘둘리지 않는 차분한 결",
    elementary: "섬세한 자극보다 큰 그림·행동에 더 끌리는 결",
  ...
  창의성: {
    infant: "익숙한 방식·반복된 자극에서 안정감을 얻는 결",
    preschool: "정해진 틀·익숙한 놀이에서 편안한 결",
    elementary: "새 발상보다 정리된 절차·규칙에서 빛나는 결",
    secondary: "새 기획보다 정립된 길을 따라가는 데 강점이 있는 결",
  },
  자기조절: {
    infant: "충동·욕구가 솔직하게 드러나는 자유로운 결",
    preschool: "차례·기다림에 부모님의 따뜻한 안내가 도움되는 결",
    elementary: "자기 페이스 조율에 부모님의 짚어줌이 도움되는 결",
    secondary: "자기 관리에 외부 구조·기준이 도움되는 결",
  },
};
...
export function buildSixFactorBodyContext(
  ...
  const weakPhrase = SIX_FACTOR_WEAK_PHRASE[weakest]?.[stage] ?? "다른 결은 부드러운 편";
  const lineWeak = `반면 ${weakPhrase}으로, 자녀 고유의 균형을 보여주고 있습니다.`;
